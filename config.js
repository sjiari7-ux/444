/* ============================================================
   CUSTOM ART (from the old game's icons/ folder — deploy this
   file next to that same icons/ folder for these to show; if a
   file is missing, .ui-icon <img> tags quietly fall back to their
   alt text via the document-level error listener below, and the
   CSS zone-banner photos just fall back to the gradient beneath).
   ============================================================ */
const CLASS_ICONS = { warrior:'icons/class_warrior.png', archer:'icons/class_archer.png', mage:'icons/class_mage.png', merchant:'icons/class_merchant.png', commander:'icons/class_shield.png' };
const KINGDOM_ICONS = { europe:'icons/kingdom_europe.png', asia:'icons/kingdom_asia.png', africa:'icons/kingdom_africa.png', north_america:'icons/kingdom_north_america.png', arab_world:'icons/kingdom_arab_world.png', south_america:'icons/kingdom_south_america.png' };
const RESOURCE_ICONS = { wood:'icons/resource_wood.png', stone:'icons/resource_stone.png', food:'icons/resource_food.png', coal:'icons/resource_coal.png', iron:'icons/resource_iron.png', herbs:'icons/resource_herbs.png', leather:'icons/resource_leather.png' };
const ZONE_PHOTOS = { plains:'icons/zone_plains.jpg', forest:'icons/zone_forest.jpg', mountain:'icons/zone_mountain.jpg', cave:'icons/zone_cave.jpg', swamp:'icons/zone_swamp.jpg', darkzone:'icons/zone_dark.jpg' };
function imgIcon(src, alt, extraClass){ return src ? `<img class="ui-icon ${extraClass||''}" src="${src}" alt="${esc(alt||'')}">` : ''; }
document.addEventListener('error', e=>{
  const el = e.target;
  if(el.tagName==='IMG' && el.classList && el.classList.contains('ui-icon')){
    if(el.alt) el.replaceWith(document.createTextNode(el.alt));
    else el.remove();
  }
}, true);
...
function generalSkillCost(currentLevel){ return 1 + Math.floor(currentLevel/5); } // skill points, same currency as class skills
