...
async function signInWithGoogle(){
  ...
  if (typeof firebase === 'undefined') {
    showError(`<img class="ui-icon" src="${ICONS.warning}" alt="⚠"> لم يتم تحميل مكتبة Firebase. تأكد أن متصفحك أو أي أداة حجب إعلانات (مثل Brave Shields أو AdBlock) لا تمنع gstatic.com`);
    return;
  }
  ...
  checkTimeout = setTimeout(async () => {
    try{
      ...
      if (db) {
        ...
        if(doc.exists){
          errorBox2.textContent = `<img class="ui-icon" src="${ICONS.cancel}" alt="❌"> This name is already taken`;
          errorBox2.style.display = 'block';
...
async function finishSetup(){
  ...
  try{
    ...
    if (err.code === 'username-taken') {
      ...
      const successBox = document.getElementById('inputSuccess');
      const nextBtn = document.getElementById('nextBtn');
      if (errorBox2) { errorBox2.textContent = `<img class="ui-icon" src="${ICONS.cancel}" alt="❌"> ` + err.message; errorBox2.style.display = 'block'; }
