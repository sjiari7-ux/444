  ...
  try{
    ...
    snap.docs.forEach(doc=>{
      ...
      if(typeof addNotification === 'function'){
        addNotification('market', `<img class="ui-icon" src="${ICONS.business}" alt="💰"> Item Sold!`, `${escapeHtml(s.buyerName||'A player')} bought ${s.qty} ${escapeHtml(itemName)} for ${s.cost}g.`);
      }
...
function setGearMarketView(view){
  ...
  else if(view === 'browse') loadGearMarketListings();
  // 'mygear' reads straight from state.gearBag — nothing to fetch
}
...
async function applyPendingGearMarketSales(){
  ...
  try{
    ...
    snap.docs.forEach(doc=>{
      const s = doc.data();
      if(typeof addNotification === 'function'){
        addNotification('market', `<img class="ui-icon" src="${ICONS.damage_ui}" alt="⚔"> Gear Sold!`, `${escapeHtml(s.buyerName||'A player')} bought your ${escapeHtml(s.gearName||'item')} for ${s.price}g.`);
      }
      batch.delete(doc.ref);
    });
    await batch.commit();
...
function refreshOpenGearMarketViews(){
  if(state.marketCategory !== 'gear') return;
  if(state.gearMarketView === 'mine') loadMyGearListings();
  else if(state.gearMarketView === 'browse') loadGearMarketListings();
  // 'mygear' is just state.gearBag — always current, nothing to poll
}
...
// Switches between the two sections of the Player Market panel —
// Resources (stackable goods, `marketListings`) and Gear (unique
// crafted items, `gearListings`) — and loads whichever data that
// section's current sub-view (browse/mine) needs.
function setMarketCategory(category){
  state.marketCategory = category;
  renderBody();
  if(category === 'gear'){
    if(state.gearMarketView === 'mine') loadMyGearListings();
    else if(state.gearMarketView === 'mygear') { /* local bag data, nothing to fetch */ }
    else loadGearMarketListings();
  } else {
    if(state.marketView === 'mine') loadMyListings();
    else if(state.marketDetailItem) loadListingsFor(state.marketDetailItem);
  }
}
