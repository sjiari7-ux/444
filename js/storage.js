...
async function loadKingdomView(){
  ...
  try{
    ...
    if(c.kingdomId){
      ...
      const snaps = await Promise.all(KINGDOMS.map(k=> withTimeout(DB.doc('rc_kingdoms/'+k.id).get(), 6000).catch(()=>({exists:false}))));
      const counts = await Promise.all(KINGDOMS.map(k=> withTimeout(DB.collection('rc_players').where('kingdomId','==',k.id).limit(500).get(), 6000).then(s=>s.size).catch(()=>0)));
