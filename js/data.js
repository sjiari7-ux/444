...
const CLASS_AVATAR_EMOJI = { warrior:'⚔️', archer:'🏹', mage:'🔮', commander:'🎖️', merchant:'💰' };

/* ===== CLASS RESOURCE MECHANICS =====
   Each class has its own resource loop instead of being a reskinned
   percentage bonus (see the chat: "ma bghinash Warrior = +20% Attack").
   - warrior/archer resources are battle-only meters (reset every fight).
   - mage "overloads" its existing Mana instead of a new meter.
   - commander/merchant resources are PERSISTENT (saved to state + synced
     to Firestore), because they're meant to carry weight outside a single
     battle: Command Points feed Kingdom Wars, Fortune is a streak you can
     lose by dying.
   `gainOnHit`/`gainOnHurt` are consumed by combat.js; `ultimate` is the
   button that appears once the meter is maxed. */
const CLASS_RESOURCE = {
  warrior: {
    key: 'rage', name: 'Rage', nameAr: 'Rage', max: 100, persistent: false,
    icon: `<img class="ui-icon" src="${ICONS.fire_streak}" alt="🔥">`, color: '#e2554a',
    gainOnHit: 10, gainOnCrit: 18, gainOnHurt: 9,
    ultimate: { key: 'berserkerStrike', name: 'Berserker Strike', nameAr: 'Berserker Strike',
      icon: `<img class="ui-icon" src="${ICONS.fire_streak}" alt="🔥">`,
      desc: 'Dump all your Rage into one guaranteed-crit strike for massive damage.' },
  },
  archer: {
    key: 'precision', name: 'Precision', nameAr: 'Precision', max: 5, persistent: false,
    icon: `<img class="ui-icon" src="${ICONS.eye}" alt="👁">`, color: '#4fb8a6',
    gainOnHit: 1, gainOnCrit: 1, resetOnHurtCrit: true,
    ultimate: { key: 'perfectShot', name: 'Perfect Shot', nameAr: 'Perfect Shot',
      icon: `<img class="ui-icon" src="${ICONS.eye}" alt="👁">`,
      desc: 'Cash in a full Precision chain for a guaranteed crit that ignores all armor.' },
  },
  mage: {
    key: 'mana', name: 'Mana Overload', nameAr: 'Mana Overload', max: null, persistent: true,
    icon: `<img class="ui-icon" src="${ICONS.mana}" alt="🔮">`, color: '#a679c9',
    overload: { key: 'overloadBlast', name: 'Overload Blast', nameAr: 'Overload Blast',
      icon: `<img class="ui-icon" src="${ICONS.sparkle}" alt="✨">`,
      desc: 'Burn double Mana for a huge Fireball — drain yourself dry and you go Exhausted.' },
  },
  commander: {
    key: 'command', name: 'Command Points', nameAr: 'Command Points', max: 100, persistent: true,
    icon: `<img class="ui-icon" src="${ICONS.crown}" alt="🎖️">`, color: '#d4a24c',
    gainOnHit: 5, gainOnCrit: 8, gainOnWin: 10,
    ultimate: { key: 'rally', name: 'Rally', nameAr: 'Rally',
      icon: `<img class="ui-icon" src="${ICONS.flag_black}" alt="🏴">`,
      desc: 'Spend Command Points for a battle buff — the same points also power bonus damage in Kingdom Wars.' },
  },
  merchant: {
    key: 'fortune', name: 'Fortune', nameAr: 'Fortune', max: 100, persistent: true,
    icon: `<img class="ui-icon" src="${ICONS.clover}" alt="🍀">`, color: '#e0a458',
    gainOnLootValue: 0.6, gainOnWin: 6, lossOnDefeat: 35,
    ultimate: { key: 'luckyDraw', name: 'Lucky Draw', nameAr: 'Lucky Draw',
      icon: `<img class="ui-icon" src="${ICONS.clover}" alt="🍀">`,
      desc: 'Cash in your Fortune streak for a guaranteed rare drop plus bonus gold.' },
  },
};
...
const CLASS_DATA = {
  warrior: {
    ...
    desc: 'Balanced damage & defense — builds Rage from every hit taken or landed, then unloads it in a Berserker Strike.',
  ...
  archer: {
    ...
    desc: 'High damage, speed & evasion — stacks Precision with every clean hit, but a monster crit against you resets the chain.',
  ...
  mage: {
    ...
    desc: 'Extreme damage, very low health — can Overload a spell to burn double Mana for a huge payoff, risking Exhaustion.',
  ...
  commander: {
    name: 'Commander', nameAr: 'Commander', icon: `<img class="ui-icon" src="${ICONS.crown}" alt="🎖️">`, color: '#d4a24c',
    desc: 'Tanky, war-focused — earns Command Points from every fight, spendable on a battle Rally or as bonus damage in Kingdom Wars & Core sieges.',
    stats: { hp: 1.35, atk: 1.05, def: 1.30, spd: 0.95, dodge: 0.85, crit: 0.90 },
    skills: [
      { key: 'warBanner', name: 'War Banner', nameAr: 'War Banner', icon: `<img class="ui-icon" src="${ICONS.flag_black}" alt="🏴">`, desc: 'Raise the banner for a heavy strike (also boosts your own damage)', perLevel: { dmgBonus: 0.035 } },
      { key: 'ironWill', name: 'Iron Will', nameAr: 'Iron Will', icon: `<img class="ui-icon" src="${ICONS.class_shield}" alt="🛡">`, desc: 'Increases defense and damage reduction', perLevel: { defBonus: 2, drBonus: 0.025 } },
      { key: 'commandAura', name: 'Command Aura', nameAr: 'Command Aura', icon: `<img class="ui-icon" src="${ICONS.crown}" alt="🎖️">`, desc: 'Increases max HP and how fast you earn Command Points', perLevel: { hpBonus: 6, commandGainBonus: 0.08 } },
    ],
    starterGear: { weapon: { tier: 0, name: 'Officer Blade', slot: 'weapon', stats: { damage: 3 } }, armor: { tier: 0, name: 'Banner Plate', slot: 'armor', stats: { defense: 0.06, health: 12 } } }
  },
  merchant: {
    name: 'Merchant', nameAr: 'Merchant', icon: `<img class="ui-icon" src="${ICONS.business}" alt="💰">`, color: '#e0a458',
    desc: 'Economic bonuses, balanced stats — builds a Fortune streak from loot and wins, but losing a fight knocks a big chunk off it.',
    ...
    skills: [
      ...
      { key: 'lucky', name: 'Lucky', nameAr: 'Lucky', icon: `<img class="ui-icon" src="${ICONS.clover}" alt="🍀">`, desc: 'Extra loot chance from battles, and builds Fortune faster', perLevel: { lootBonus: 0.03 } },
    ],
