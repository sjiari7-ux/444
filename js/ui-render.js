...
function ppRankForLevel(lvl){
  ...
  if(lvl >= 35) return { name:'Champion', icon:'🏆' };
...
function buildPlayerProfileHtml(p, isSelf){
  ...
  const medallions = [
    { icon:'🪙', val:fmtG(p.gold), label:'Gold', color:'var(--brass-bright)' },
    { icon:'⚔️', val:p.wins, label:'Wins', color:'var(--green)' },
    { icon:'💀', val:p.losses, label:'Losses', color:'var(--red)' },
    { icon:'✨', val:p.prestigePoints || 0, label:'Prestige', color:'var(--prestige)' },
    { icon:'💰', val:fmtG(p.totalGoldEarned || p.gold), label:'Earned', color:'var(--brass-bright)' },
    { icon:'🎒', val:p.gearCount || 0, label:'Gear', color:'var(--skill)' },
  ].map(m => `
