...
function renderLeaderboard(){
  ...
  if(leaderboardError && !leaderboardByGold){
    return `
      <div class="lb-note"><img class="ui-icon" src="${ICONS.warning}" alt="⚠"> ${leaderboardError}</div>
      <button class="btn btn-secondary" style="margin-top:8px;" onclick="refreshLeaderboard()">Retry</button>`;
  }
  const rowsFor = (list, valueKey)=> {
    if(!list.length) return `<div class="lb-note">No players ranked yet.</div>`;
    return `<div class="rank-list">${list.map((r,i)=>{
      const rank = i+1;
      const tierClass = rank===1?'top1':rank===2?'top2':rank===3?'top3':'';
      const val = valueKey==='gold' ? fmtG(r.gold) : valueKey==='level' ? r.level : valueKey==='pvpRating' ? r.pvpRating : r.pvpWins;
      const unit = valueKey==='gold' ? 'g' : valueKey==='level' ? '' : valueKey==='pvpRating' ? '' : '';
      const sub = valueKey==='pvpRating' ? pvpTierFor(r.pvpRating).name : '';
      return `<div class="rank-row ${tierClass} ${r.me?'me':''}" onclick="viewChatProfile('${r.uid}')">
        ...
        <span class="rank-id">
          <span class="rank-name">${escapeHtml(r.me?'You':r.name)}</span>
          ${sub?`<span class="rank-sub">${sub}</span>`:''}
        </span>
        <span class="rank-value">${val}<span class="unit">${unit}</span></span>
      </div>`;
    }).join('')}</div>`;
  };
  return `
    <div class="lb-note"><img class="ui-icon" src="${ICONS.globe}" alt="🌐"> Live server-wide leaderboard, top ${LEADERBOARD_SIZE} players.
      <span style="cursor:pointer;color:var(--brass-bright);" onclick="refreshLeaderboard()"><img class="ui-icon" src="${ICONS.reload}" alt="↻"> Refresh</span>
    </div>
    ${renderKingdomLeaderboardPanel()}
    <div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(240px,1fr));">
      <div class="panel"><div class="section-title" style="margin-top:0;"><h2><img class="ui-icon" src="${ICONS.medal_gold}" alt="🥇"> Richest</h2></div>${rowsFor(leaderboardByGold||[],'gold')}</div>
      <div class="panel"><div class="section-title" style="margin-top:0;"><h2><img class="ui-icon" src="${ICONS.medal_silver}" alt="🥈"> Highest Level</h2></div>${rowsFor(leaderboardByLevel||[],'level')}</div>
      <div class="panel"><div class="section-title" style="margin-top:0;"><h2><img class="ui-icon" src="${ICONS.damage_ui}" alt="⚔"> Top PvP Rating</h2><span style="cursor:pointer;font-size:11px;color:var(--brass-bright);" onclick="openPvpRankTop100()">Top 100 →</span></div>${rowsFor(leaderboardByPvpRating||[],'pvpRating')}</div>
      <div class="panel"><div class="section-title" style="margin-top:0;"><h2><img class="ui-icon" src="${ICONS.damage_ui}" alt="⚔"> Most PvP Wins</h2></div>${rowsFor(leaderboardByPvpWins||[],'pvpWins')}</div>
    </div>`;
}
