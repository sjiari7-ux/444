...
function renderBodyHTML(){
  ...
  switch(activeTab){
    ...
    case 'battles': html = renderActiveWarsList(); break;
    default: html = ''; break;
  }
  if(typeof activeWarModalId !== 'undefined' && activeWarModalId) html += renderWarDetailModal();
  if(typeof activeCoreModalOpen !== 'undefined' && activeCoreModalOpen) html += renderCoreDetailModal();
  if(typeof activeSeasonHistoryOpen !== 'undefined' && activeSeasonHistoryOpen) html += renderSeasonHistoryModal();
  return html;
}
...
function renderProduction(){
  ...
  return `
    <div class="section-title"><h2><img class="ui-icon" src="${ICONS.tools}" alt="🛠"> Crafting</h2><span class="rule"></span><div class="sub">Backpack ${fmtG(getTotalStorageUsed(state))}/${fmtG(cap)}</div></div>
    ${renderCrafting()}
    <div class="panel" style="margin-top:14px;">
      <div class="panel-header"><img class="ui-icon" src="${ICONS.meat}" alt="🍖"> Recovery</div>
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:12px;">
        <button class="act-btn copper" style="width:auto;padding:9px 18px;" ${state.inv.food<5?'disabled':''} onclick="eat()"><img class="ui-icon" src="${ICONS.bread}" alt="🍞"> Eat 5 Food (+5<img class="ui-icon" src="${ICONS.energy}" alt="⚡">)</button>
        <button class="act-btn red" style="width:auto;padding:9px 18px;" ${(state.inv.food<5||state.health>=maxH)?'disabled':''} onclick="healWithFood()"><img class="ui-icon" src="${ICONS.bandage}" alt="🩹"> Heal (+20<img class="ui-icon" src="${ICONS.heart_hp}" alt="❤️">)</button>
      </div>
      <div style="margin-top:14px;padding-top:14px;border-top:1px solid var(--border);">
        <div style="font-size:11px;color:var(--dim);text-transform:uppercase;letter-spacing:0.08em;margin-bottom:10px;"><img class="ui-icon" src="${ICONS.bread}" alt="🍞"> Crafted Consumables — No Energy Cost</div>
...
function renderInventory(){
  ...
  const allCards = ownedKeys.map(key=>{
    ...
    if(isBread){
      const maxed = state.health >= getMaxHealth(state);
      effectLine = `<div class="item-effect" style="color:var(--health);"><img class="ui-icon" src="${ICONS.heart_hp}" alt="❤"> +${BREAD_TIERS[key].heal} HP</div>`;
      consumeBtn = `<button class="mini-btn" style="border-color:var(--health);color:var(--health);margin-top:8px;width:100%;" ${maxed?'disabled':''} onclick="consumeBread('${key}')">${maxed?'HP Full':'<img class="ui-icon" src="${ICONS.plate}" alt="🍽"> Eat'}</button>`;
    } else if(isEnergy){
      const maxed = state.energy >= getMaxEnergy(state);
      effectLine = `<div class="item-effect" style="color:var(--brass-bright);"><img class="ui-icon" src="${ICONS.energy}" alt="⚡"> +${ENERGY_POTION_TIERS[key].energy} Energy</div>`;
      consumeBtn = `<button class="mini-btn" style="border-color:var(--brass-bright);color:var(--brass-bright);margin-top:8px;width:100%;" ${maxed?'disabled':''} onclick="consumeEnergyPotion('${key}')">${maxed?'Energy Full':'<img class="ui-icon" src="${ICONS.potion}" alt="🧪"> Drink'}</button>`;
    } else if(isHealthPotion){
      const maxed = state.health >= getMaxHealth(state);
      effectLine = `<div class="item-effect" style="color:var(--health);"><img class="ui-icon" src="${ICONS.heart_hp}" alt="❤"> +30-50 HP</div>`;
      consumeBtn = `<button class="mini-btn" style="border-color:var(--health);color:var(--health);margin-top:8px;width:100%;" ${maxed?'disabled':''} onclick="consumeBread('${key}')">${maxed?'HP Full':'<img class="ui-icon" src="${ICONS.plate}" alt="🍽"> Use'}</button>`;
    }
  ...
  const emptyState = ownedKeys.length === 0 ? `
    <div class="panel" style="text-align:center;padding:40px 20px;">
      <div style="font-size:48px;margin-bottom:12px;"><img class="ui-icon" src="${ICONS.bag_full}" alt="🎒"></div>
...
function renderCrafting(){
  ...
  const lockedSection = lockedRows.length ? `
    <div class="section-title" style="margin-top:18px;"><h2><img class="ui-icon" src="${ICONS.lock}" alt="🔒"> Locked</h2><span class="rule"></span><div class="sub">${lockedRows.length} recipe${lockedRows.length===1?'':'s'}</div></div>
  ...
  return `
    <div class="panel" style="padding:0;overflow:hidden;">
      ...
      <div style="display:flex;gap:8px;padding:12px 16px;border-bottom:1px solid var(--border);background:var(--panel);">
        <button class="market-tab-btn ${category==='resources'?'active':''}" onclick="setMarketCategory('resources')"><img class="ui-icon" src="${ICONS.tree_pine}" alt="🌲"> Resources</button>
        <button class="market-tab-btn ${category==='gear'?'active':''}" onclick="setMarketCategory('gear')"><img class="ui-icon" src="${ICONS.damage_ui}" alt="⚔"> Gear</button>
      </div>
      <div style="display:flex;gap:8px;padding:12px 16px;border-bottom:1px solid var(--border);background:var(--panel);flex-wrap:wrap;">
        <button class="market-tab-btn ${view==='browse'?'active':''}" onclick="${isGear?"setGearMarketView('browse')":"setMarketView('browse')"}"><img class="ui-icon" src="${ICONS.cart}" alt="🛒"> Browse</button>
        <button class="market-tab-btn ${view==='mine'?'active':''}" onclick="${isGear?"setGearMarketView('mine')":"setMarketView('mine')"}"><img class="ui-icon" src="${ICONS.tag}" alt="🏷"> My Listings</button>
        ${isGear ? `<button class="market-tab-btn ${view==='mygear'?'active':''}" onclick="setGearMarketView('mygear')"><img class="ui-icon" src="${ICONS.bag_full}" alt="🎒"> My Gear</button>` : ''}
      </div>
      ${body}
    </div>
...
function renderGearMarketSection(){
  ...
  const filterButtons = ['all', ...Object.keys(GEAR_SLOTS)].map(slot=>{
    ...
    const icon = slot === 'all' ? `<img class="ui-icon" src="${ICONS.card_index}" alt="🗂">` : GEAR_SLOTS[slot].icon;
...
// (Market <img class="ui-icon" src="${ICONS.arrow_right}" alt="→"> Gear) or, previously, a "Weapon Goods" tradeable-goods filter
  ...
  const rows = filteredKeys.map(key=>{
    ...
    return `<div class="market-row">
      ...
      <div class="market-actions">
        <button class="mini-btn buy" onclick="openMarketDetail('${key}')"><img class="ui-icon" src="${ICONS.cart}" alt="🛒"> Browse offers</button>
        <button class="mini-btn sell" ${owned>0?'':'disabled'} onclick="openSellModal('${key}')"><img class="ui-icon" src="${ICONS.tag}" alt="🏷"> Sell</button>
      </div>
    </div>`;
  }).join('');
  return `
    <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;padding:12px 16px;border-bottom:1px solid var(--border);background:var(--panel);">
      ...
      <button class="mini-btn" onclick="clearMarketFilters()" style="padding:8px 12px;"><img class="ui-icon" src="${ICONS.close}" alt="✕"> Clear</button>
    </div>
    ${rows}`;
}
...
function renderMyListingsView(){
  ...
  if(!state.myListingsCache.length){
    return `<div style="padding:30px 20px;text-align:center;color:var(--dim);font-size:13px;">You have nothing listed. Go to Browse and hit <img class="ui-icon" src="${ICONS.tag}" alt="🏷"> Sell on anything you own.</div>`;
  }
  const rows = state.myListingsCache.map(l=>{
    ...
    return `<div class="market-row">
      ...
      <div class="market-actions">
        <button class="mini-btn sell" onclick="cancelListing('${l.id}')"><img class="ui-icon" src="${ICONS.close}" alt="✕"> Cancel & return items</button>
      </div>
    </div>`;
  }).join('');
  return rows;
...
function renderMarketDetailModal(key){
  ...
  return `
    <div class="modal-overlay" onclick="if(event.target===this)closeMarketDetail()">
      <div class="modal-box">
        <div class="modal-header"><h3>${it.icon} ${escapeHtml(it.name)} — offers</h3><button class="modal-close" onclick="closeMarketDetail()"><img class="ui-icon" src="${ICONS.close}" alt="✕"></button></div>
...
function renderSellModal(key){
  ...
  const mineRows = mine.length ? mine.map(l => `
    <div class="market-row" style="padding:8px 0;">
      ...
      <div class="market-actions"><button class="mini-btn sell" onclick="cancelListing('${l.id}')"><img class="ui-icon" src="${ICONS.close}" alt="✕"> Cancel</button></div>
    </div>`).join('') : '';
  return `
    <div class="modal-overlay" onclick="if(event.target===this)closeSellModal()">
      <div class="modal-box">
        <div class="modal-header"><h3>${it.icon} Sell ${escapeHtml(it.name)}</h3><button class="modal-close" onclick="closeSellModal()"><img class="ui-icon" src="${ICONS.close}" alt="✕"></button></div>
...
function renderCompanies(){
  ...
  if(state.companyBuildResource !== null){
    ...
    const costText = isFirstCompany 
      ? `<span class="resource-chip" style="border-color:var(--green);color:var(--green);"><img class="ui-icon" src="${ICONS.gift}" alt="🎁"> Free!</span>`
      : `<span class="resource-chip" style="${hasConcrete?'border-color:var(--green);color:var(--green);':'border-color:var(--red);color:var(--red);'}"><img class="ui-icon" src="${ICONS.brick}" alt="🧱"> ${nextConcreteCost} Concrete</span>`;

    buildModal = `
      <div class="modal-overlay" onclick="if(event.target===this)cancelBuild()">
        <div class="modal-box">
          <div class="modal-header"><h3><img class="ui-icon" src="${ICONS.construction}" alt="🏗"> Build New Company</h3><button class="modal-close" onclick="cancelBuild()"><img class="ui-icon" src="${ICONS.close}" alt="✕"></button></div>
          <div class="modal-body">
            ...
            <div style="margin-top:14px;text-align:center;">
              ...
              <button class="act-btn buy" style="width:100%;padding:12px;" ${hasConcrete?'':'disabled'} onclick="buildCompany()">
                ${hasConcrete ? `<img class="ui-icon" src="${ICONS.construction}" alt="🏗"> Build ${it.name} Company` : 'Not Enough Concrete'}
              </button>
            </div>
          </div>
        </div>
  ...
  const companyCards = state.companies.map((c, idx) => {
    ...
    const menuItems = [];
    menuItems.push({icon:`<img class="ui-icon" src="${ICONS.refresh}" alt="🔄">`, label:'Change production', action:`startChangeResource(${c.id});`, cls:''});

    if(canUpgrade){
      const ok = hasSteel ? `<img class="ui-icon" src="${ICONS.check}" alt="✓">` : `<img class="ui-icon" src="${ICONS.x_mark}" alt="✗">`;
    ...
    return `<div class="company-card" style="${isDisabled?'opacity:0.5;':''}${canCollect?'border-color:rgba(255,195,92,0.3);':''}">
      ...
      <button class="act-btn buy" style="width:100%;margin-top:10px;padding:10px;" ${(!canCollect || space<=0)?'disabled':''} onclick="collectFromCompany(${c.id})">
        ${space<=0?`<img class="ui-icon" src="${ICONS.cancel}" alt="❌"> Storage Full`:(canCollect?`<img class="ui-icon" src="${ICONS.bag_full}" alt="📦"> Collect ${Math.min(Math.floor(c.stored),space)} ${it.icon}`:'⏳ Nothing to Collect')}
      </button>
  ...
  const buildSection = currentCount >= MAX_COMPANIES ? '' : `
    <div class="panel" style="text-align:center;padding:24px;background:radial-gradient(ellipse at center,rgba(255,195,92,0.06) 0%,transparent 70%);">
      <div style="font-size:36px;margin-bottom:8px;"><img class="ui-icon" src="${ICONS.construction}" alt="🏗"></div>
      ...
      ${canBuild ? `
        <div style="font-size:12px;color:var(--dim);margin-bottom:10px;">
          ${currentCount === 0 ? `<span class="resource-chip" style="border-color:var(--green);color:var(--green);"><img class="ui-icon" src="${ICONS.gift}" alt="🎁"> First company is FREE</span>` : `<span class="resource-chip"><img class="ui-icon" src="${ICONS.brick}" alt="🧱"> ${nextConcreteCost} Concrete</span>`}
          ${currentCount >= 2 ? `<span style="margin-left:8px;" class="resource-chip"><img class="ui-icon" src="${ICONS.business}" alt="🏭"> Lv.${currentCount-1} Mgmt</span>` : ''}
        </div>
        <button class="act-btn buy" style="width:auto;padding:10px 24px;" onclick="openBuildSelector()"><img class="ui-icon" src="${ICONS.construction}" alt="🏗"> Build Company</button>
  ...
  return `
    ${buildModal}
    <div class="section-title"><h2><img class="ui-icon" src="${ICONS.business}" alt="🏭"> Your Companies</h2><div class="sub">${currentCount}/${MAX_COMPANIES} companies · Collect below · ⋯ for more · 1-9 tabs</div></div>
    ${state.companies.length === 0 ? `
      <div class="panel" style="text-align:center;padding:32px;">
        <div style="font-size:48px;margin-bottom:12px;"><img class="ui-icon" src="${ICONS.business}" alt="🏭"></div>
        ...
        ${canBuild ? `
          ...
          <button class="act-btn buy" style="width:auto;padding:12px 28px;font-size:14px;" onclick="openBuildSelector()"><img class="ui-icon" src="${ICONS.construction}" alt="🏗"> Build First Company</button>
...
function renderClass(){
  const backLink = `<button class="mini-btn" style="margin-bottom:12px;" onclick="activeTab='settings';renderBody();"><img class="ui-icon" src="${ICONS.arrow_left}" alt="←"> Back to Settings</button>`;
  ...
  const skillCards = cls.skills.map((sk, idx)=>{
    ...
    Object.keys(sk.perLevel).forEach(ef=>{
      ...
      else if(ef==='commandGainBonus') label = `+${(total*100).toFixed(0)}% Command Point gain`;
      effects.push(label);
    });
    ...
    return `<div class="skill-card-v2 ${canUpgrade?'upgradable':''}" style="--sk:${cls.color};">
      ...
      ${maxed ? `<div class="sk-maxed"><img class="ui-icon" src="${ICONS.sparkle}" alt="✨"> Maxed</div>` : `<button class="sk-upgrade-btn" ${canUpgrade?'':'disabled'} onclick="upgradeClassSkill('${sk.key}')">UPGRADE · ${nextCost} CSP</button>`}
    </div>`;
  }).join('');
  ...
  return `
    ...
    <div class="stat-overview-grid">
      <div class="stat-overview-box"><div class="label"><img class="ui-icon" src="${ICONS.heart_hp}" alt="❤"> HP</div><div class="value">${(stats.hp*100).toFixed(0)}%</div></div>
      <div class="stat-overview-box"><div class="label"><img class="ui-icon" src="${ICONS.damage_ui}" alt="⚔"> ATK</div><div class="value">${(stats.atk*100).toFixed(0)}%</div></div>
      <div class="stat-overview-box"><div class="label"><img class="ui-icon" src="${ICONS.defense_ui}" alt="🛡"> DEF</div><div class="value">${(stats.def*100).toFixed(0)}%</div></div>
      <div class="stat-overview-box"><div class="label"><img class="ui-icon" src="${ICONS.dash}" alt="💨"> SPD</div><div class="value">${(stats.spd*100).toFixed(0)}%</div></div>
      <div class="stat-overview-box"><div class="label"><img class="ui-icon" src="${ICONS.spellbook}" alt="🎯"> CRIT</div><div class="value">${(stats.crit*100).toFixed(0)}%</div></div>
      <div class="stat-overview-box"><div class="label"><img class="ui-icon" src="${ICONS.cyclone}" alt="🌀"> DODGE</div><div class="value">${(stats.dodge*100).toFixed(0)}%</div></div>
    </div>
    ...
    <div class="panel" style="margin-top:16px;text-align:center;padding:20px;">
      <div style="font-size:24px;margin-bottom:8px;"><img class="ui-icon" src="${ICONS.refresh}" alt="🔄"></div>
      ...
      <div style="display:flex;gap:6px;justify-content:center;flex-wrap:wrap;margin-bottom:12px;">
        ${Object.keys(CLASS_DATA).filter(k=>k!==state.playerClass).map(k=>{
          ...
          return `<button class="mini-btn" style="border-color:${c.color};color:${c.color};" ${canReset?'':'disabled'} onclick="if(confirm('Reset to ${c.nameAr} for ${fmtG(resetCost)}g?'))resetClass('${k}')">${c.icon} ${c.nameAr}</button>`;
        }).join('')}
      </div>
...
function renderSkills(){
  ...
  const overview = [
    ...
    { icon:`<img class="ui-icon" src="${ICONS.business}" alt="💰">`, label:'Profit', value: `+${((getSellMult(state)-1)*100).toFixed(0)}%` },
  ...
  const cards = Object.keys(SKILLS).map(key=>{
    ...
    return `<div class="skill-card-v2 ${canUpgrade?'upgradable':''}" style="--sk:${color};">
      ...
      <div class="sk-effects">${effectsHtml}</div>
      ${maxed
        ? `<div class="sk-maxed"><img class="ui-icon" src="${ICONS.sparkle}" alt="✨"> Maxed</div>`
        : `<button class="sk-upgrade-btn" ${canUpgrade?'':'disabled'} onclick="upgradeSkill('${key}')">UPGRADE · ${cost} SP</button>`}
    </div>`;
  }).join('');

  return `
    <button class="mini-btn" style="margin-bottom:12px;" onclick="activeTab='settings';renderBody();"><img class="ui-icon" src="${ICONS.arrow_left}" alt="←"> Back to Settings</button>
...
function renderMissions(){
  ...
  const tabs = [
    { id:'daily', label:'Daily', icon:`<img class="ui-icon" src="${ICONS.calendar}" alt="📅">`, data: mData.daily, pool: DAILY_MISSIONS },
    { id:'weekly', label:'Weekly', icon:`<img class="ui-icon" src="${ICONS.calendar}" alt="📅">`, data: mData.weekly, pool: WEEKLY_MISSIONS },
    { id:'monthly', label:'Monthly', icon:`<img class="ui-icon" src="${ICONS.moon}" alt="🌙">`, data: mData.monthly, pool: MONTHLY_MISSIONS },
  ];

  // Compact horizontal-scroll pill tabs (fits phones without stacking
  // three full-width cards vertically before you even reach the list).
  const tabHtml = tabs.map(t => {
    ...
    return `
      <button class="mission-tab-pill ${isActive?'active':''} ${isDone?'done':''}" onclick="state.activeMissionTab='${t.id}';renderBody();">
        <span class="mtp-icon">${t.icon}</span>
        <span class="mtp-label">${t.label}</span>
        ${isDone
          ? `<span class="mtp-check"><img class="ui-icon" src="${ICONS.check}" alt="✓"></span>`
          : `<span class="mtp-count">${completed}/${total}</span>`}
      </button>
    `;
  }).join('');
  ...
  const data = current.data || { progress:{}, claimed:[], lastReset:0 };

  // Summary strip for whichever tab is active — one compact row instead
  // of repeating a progress bar + timer inside every tab pill.
  const summaryCompleted = data.claimed ? data.claimed.length : 0;
  const summaryTotal = pool.length;
  const summaryPct = summaryTotal > 0 ? (summaryCompleted/summaryTotal)*100 : 0;
  const summaryHtml = `
    <div class="mission-summary">
      <div class="mission-summary-bar-track">
        <div class="mission-summary-bar-fill" style="width:${summaryPct}%"></div>
      </div>
      <span class="mission-summary-count">${summaryCompleted}/${summaryTotal}</span>
      <span class="mission-summary-timer">⏳ ${humanCountdown(data.lastReset||0)}</span>
    </div>
  `;
  ...
  const rows = pool.map(m => {
    ...
    const rewards = [];
    if(reward.xp) rewards.push(`<span class="mission-reward-xp"><img class="ui-icon" src="${ICONS.sparkle}" alt="✨"> ${reward.xp}</span>`);
        ...
        <div class="mission-action">
          ${isClaimed 
            ...
            : isComplete 
              ...
              : `<button class="mission-btn arrow" onclick="goToMissionTarget('${m.track}')"><img class="ui-icon" src="${ICONS.arrow_right}" alt="→"></button>`
          }
        </div>
      </div>
    `;
  ...
  return `
    <div class="missions-container">
      ...
      <div class="missions-tabs-scroll">${tabHtml}</div>
      ${summaryHtml}
...
let leaderboardSubTab = 'citizens'; // 'citizens' | 'kingdoms'
function setLeaderboardSubTab(t){ leaderboardSubTab = t; renderBody(); }

function renderLeaderboard(){
  if(!db){
    return `<div class="lb-note"><img class="ui-icon" src="${ICONS.offline}" alt="🔌"> Leaderboard needs a cloud connection. Sign in with Google to compete on the real, server-wide leaderboard.</div>`;
  }
  const tabs = `<div class="lb-subtabs">
    <button class="lb-subtab ${leaderboardSubTab==='citizens'?'active':''}" onclick="setLeaderboardSubTab('citizens')"><img class="ui-icon" src="${ICONS.group}" alt="👥"> Citizens</button>
    <button class="lb-subtab ${leaderboardSubTab==='kingdoms'?'active':''}" onclick="setLeaderboardSubTab('kingdoms')"><img class="ui-icon" src="${ICONS.castle}" alt="🏰"> Kingdoms</button>
  </div>`;
  const body = leaderboardSubTab === 'kingdoms'
    ? (typeof renderKingdomLeaderboardPanel === 'function' ? renderKingdomLeaderboardPanel() : '')
    : renderCitizenRankings();
  return `${tabs}${body}`;
}

function renderCitizenRankings(){
  const key = citizenLbMetric + '|' + (citizenLbKingdom || 'all');
  if(!citizenLbLoading && (!citizenLbRows || citizenLbKey !== key)) loadCitizenLeaderboard(false).then(renderBody);

  const metric = LB_METRICS[citizenLbMetric];
  const tiers = lbTiersFor(citizenLbMetric);
  const kdefSel = citizenLbKingdom ? kingdomDef(citizenLbKingdom) : null;

  const metricMenu = citizenLbMetricMenuOpen ? `<div class="lb-select-menu">${Object.keys(LB_METRICS).map(k => `
    <div class="lb-select-item ${k===citizenLbMetric?'active':''}" onclick="setCitizenLbMetric('${k}')"><img class="ui-icon" src="${LB_METRICS[k].icon()}" alt=""> ${LB_METRICS[k].label}</div>`).join('')}</div>` : '';
  const kingdomMenu = citizenLbKingdomMenuOpen ? `<div class="lb-select-menu right">
    <div class="lb-select-item ${!citizenLbKingdom?'active':''}" onclick="setCitizenLbKingdom(null)"><img class="ui-icon" src="${ICONS.globe}" alt="🌐"> All Kingdoms</div>
    ${KINGDOMS.map(k => `<div class="lb-select-item ${citizenLbKingdom===k.id?'active':''}" onclick="setCitizenLbKingdom('${k.id}')">${k.emblem} ${escapeHtml(k.name)}</div>`).join('')}
  </div>` : '';

  const tierStrip = citizenLbTierCounts ? `<div class="lb-tier-strip">${tiers.map(t => `
    <div class="lb-tier-chip" style="border-color:${t.color}55;"><div class="ltc-name" style="color:${t.color};">${t.name}</div><div class="ltc-count">${citizenLbTierCounts[t.name]||0} citizen${(citizenLbTierCounts[t.name]||0)===1?'':'s'}</div></div>`).join('')}</div>` : '';

  const myRankStrip = citizenLbMyRank ? `<div class="lb-my-rank">
    <div class="lmr-num">#${citizenLbMyRank}</div>
    <div><div class="lmr-label">Your Rank${kdefSel?` · ${escapeHtml(kdefSel.name)}`:''}</div><div style="font-size:11px;color:var(--dim);">${metric.label}</div></div>
    <div class="lmr-value">${fmtLbValue(citizenLbMetric, myMetricValue(citizenLbMetric))}</div>
  </div>` : '';

  const rows = citizenLbRows || [];
  const rowsHtml = rows.length ? `<div class="rank-list">${rows.map((r,i) => {
    const rank = i+1;
    const tierClass = rank===1?'top1':rank===2?'top2':rank===3?'top3':'';
    return `<div class="rank-row ${tierClass} ${r.me?'me':''}" onclick="viewChatProfile('${r.uid}')">
      <span class="rank-num">${rank}</span>
      <span class="rank-avatar">${escapeHtml(r.avatar||'🧙')}</span>
      <span class="rank-id"><span class="rank-name">${escapeHtml(r.me?'You':r.name)}</span></span>
      <span class="rank-value">${fmtLbValue(citizenLbMetric, metricValueOf(r, citizenLbMetric))}</span>
    </div>`;
  }).join('')}</div>` : (citizenLbLoading ? `<div class="lb-note">Loading rankings…</div>` : `<div class="lb-note">No citizens ranked yet.</div>`);

  return `
    <div class="lb-selectors">
      <div style="position:relative;flex:1;">
        <button class="lb-select-btn" onclick="toggleCitizenLbMetricMenu()"><span><img class="ui-icon" src="${metric.icon()}" alt=""> ${metric.label}</span><img class="lb-chev ui-icon" src="${ICONS.arrow_down}" alt="▾"></button>
        ${metricMenu}
      </div>
      <div style="position:relative;flex:1;">
        <button class="lb-select-btn" onclick="toggleCitizenLbKingdomMenu()"><span>${kdefSel ? `${kdefSel.emblem} ${escapeHtml(kdefSel.name)}` : 'All Kingdoms'}</span><img class="lb-chev ui-icon" src="${ICONS.arrow_down}" alt="▾"></button>
        ${kingdomMenu}
      </div>
    </div>
    ${tierStrip}
    ${myRankStrip}
    ${citizenLbError ? `<div class="lb-note"><img class="ui-icon" src="${ICONS.warning}" alt="⚠"> ${citizenLbError}</div>` : ''}
    <div class="panel">
      <div class="section-title" style="margin-top:0;display:flex;justify-content:space-between;align-items:center;">
        <h2>${metric.label} Rankings</h2>
        <span style="font-size:11px;color:var(--dim);display:flex;align-items:center;gap:6px;">Top ${LEADERBOARD_SIZE} <span style="cursor:pointer;color:var(--brass-bright);" onclick="refreshLeaderboard()"><img class="ui-icon" src="${ICONS.reload}" alt="↻"></span></span>
      </div>
      ${rowsHtml}
    </div>`;
}
...
function renderKingdomLeaderboardPanel(){
  if(kingdomLeaderboardLoading && !kingdomLeaderboard){
    return `<div class="panel" style="margin-bottom:14px;"><div class="lb-note">Loading kingdoms…</div></div>`;
  }
  if(kingdomLeaderboardError && !kingdomLeaderboard){
    return `<div class="panel" style="margin-bottom:14px;"><div class="lb-note"><img class="ui-icon" src="${ICONS.warning}" alt="⚠"> ${kingdomLeaderboardError}</div></div>`;
  }
  const list = kingdomLeaderboard || [];
  const rows = list.map((k, i) => {
    const rank = i+1;
    const tierClass = rank===1?'top1':rank===2?'top2':rank===3?'top3':'';
    return `
    <div class="rank-row ${tierClass} ${k.mine?'me':''}" onclick="focusKingdomOnWorldMap('${k.id}')" title="View on the world map">
      <span class="rank-num">${rank}</span>
      <span class="rank-avatar" style="background:${k.color};">${k.emblem}</span>
      <span class="rank-id">
        <span class="rank-name">${escapeHtml(k.name)}</span>
        <span class="rank-sub">${k.territories}/30 territories · ${k.warsWon}-${k.warsLost} sieges</span>
      </span>
      <span class="rank-value">${formatWarNumber(k.totalWarDamage)}<div class="rank-value-sub">${k.memberCount} citizens</div></span>
    </div>`;
  }).join('');
  return `
    <div class="panel" style="margin-bottom:14px;">
      <div class="section-title" style="margin-top:0;"><h2><img class="ui-icon" src="${ICONS.leaderboard_trophy}" alt="🏆"> Kingdoms at War</h2></div>
      <div class="rank-list">
        ${rows || `<div class="lb-note">No kingdoms yet.</div>`}
      </div>
    </div>`;
}
...
function renderSettings(){
  ...
  const classIcon = classInfo ? classInfo.icon : `<img class="ui-icon" src="${ICONS.compass}" alt="🧭">`;
    ...
    <div class="grid" style="grid-template-columns:1fr;">
      ...
      <div class="profile-hero" style="cursor:pointer;" onclick="showPlayerProfile()">
        ...
        <div class="profile-hero-info" style="min-width:0;">
          <span style="font-size:15px;flex-shrink:0;">${isGuest ? `<img class="ui-icon" src="${ICONS.gamepad}" alt="🎮">` : `<img class="ui-icon" src="${ICONS.envelope}" alt="✉">`}</span>
      ...
      <div>
        ...
        <div class="settings-list">
          ...
          ${settingsRow(`<img class="ui-icon" src="${ICONS.palette}" alt="🎨">`,'Appearance','Theme, language & accent','openPreferencesModal()')}
          ${settingsRow(`<img class="ui-icon" src="${ICONS.trophy}" alt="🏆">`,'Stats & Achievements','Session totals','openAchievementsModal()')}
        </div>
      </div>
      ...
      <div>
        ...
        <div class="settings-list">
          ${settingsRow(`<img class="ui-icon" src="${ICONS.lock_key}" alt="🔐">`,'Login & Security', isGuest ? 'Playing as guest' : ('Linked to '+EMAIL), 'openSecurityModal()')}
          ${settingsRow(`<img class="ui-icon" src="${ICONS.market_chart}" alt="📊">`,'Data & Backup','Export, import or clear local data','openDataModal()')}
        </div>
      </div>
      ...
      <div>
        ...
        <div class="settings-list">
          ${settingsRow(`<img class="ui-icon" src="${ICONS.sparkle}" alt="✨">`,'Prestige', canPrest ? 'Ready — Spiritual Renewal available' : `Requires level ${PRESTIGE_LEVEL_REQ}`, 'openPrestigeModal()')}
          ${settingsRow('ℹ️','About','Version, credits & support','openAboutModal()')}
          ${settingsRow(`<img class="ui-icon" src="${ICONS.trash}" alt="🗑">`,'Erase All Progress','Permanently delete saved data — cannot be undone','if(confirm(\'Erase all progress? This cannot be undone.\'))hardReset()', true)}
        </div>
      </div>
    ...
    modal.innerHTML = `
        <div class="modal-box">
            <div class="modal-header">
                <h3>${title}</h3>
                <button class="modal-close" onclick="this.closest('.modal-overlay').remove()"><img class="ui-icon" src="${ICONS.close}" alt="✕"></button>
            </div>
...
// Bump this whenever the notice text below changes, so returning players
// see the update once instead of it either nagging every load or never
// resurfacing. Shown once per browser via localStorage — reuses the same
// .modal-overlay/.modal-box as every other modal, so it's already
// responsive on phone (bottom-sheet style, see the max-width:600px rule
// in style.css) and on desktop (centered card) with no extra CSS needed.
const DEV_NOTICE_VERSION = 'v1';
function showDevNoticeModal(){
  try{
    if(localStorage.getItem('arcadia-dev-notice-seen') === DEV_NOTICE_VERSION) return;
  }catch(e){ /* localStorage unavailable (private mode etc.) — just show it */ }

  const modal = showModal(
    `<img class="ui-icon" src="${ICONS.warning || ICONS.close}" alt="⚠️"> Still in Development`,
    `
    <div style="display:flex;flex-direction:column;gap:12px;font-size:13px;line-height:1.6;color:var(--text);">
      <p style="margin:0;">
        Arcadia MMO is still an early, actively-developed build — you're
        playing a work in progress, not a finished game.
      </p>
      <ul style="margin:0;padding-inline-start:20px;display:flex;flex-direction:column;gap:6px;">
        <li>Your gold, level, gear and other stats are <b>not fully
          protected yet</b> — some values can still be tampered with, so
          leaderboards, PvP results and the marketplace may not always be
          trustworthy for now.</li>
        <li>Balance, rewards and systems (including kingdom wars) can
          still change without notice.</li>
        <li>Progress may be reset or adjusted as the security and economy
          work continues.</li>
      </ul>
      <p style="margin:0;color:var(--dim);">
        Thanks for testing early — bug reports and feedback are very
        welcome.
      </p>
    </div>
    `
  );
  try{ localStorage.setItem('arcadia-dev-notice-seen', DEV_NOTICE_VERSION); }catch(e){}
  return modal;
}
...
function openPersonalDetailsModal() {
    ...
    const body = `
        ...
        <div style="margin-bottom:14px;">
            ...
            <div style="display:flex;align-items:center;gap:10px;">
                ...
                <button class="mini-btn buy" onclick="openNameModal()"><img class="ui-icon" src="${ICONS.pencil}" alt="✏"> Change</button>
            </div>
        </div>
        <div>
            <div style="font-size:12px;color:var(--dim);margin-bottom:6px;"><img class="ui-icon" src="${ICONS.memo}" alt="📝"> Bio</div>
    ...
    `;
    showModal(`<img class="ui-icon" src="${ICONS.player_icon}" alt="👤"> Personal Details`, body);
}

function openPreferencesModal() {
    const themeOptions = `
        <button class="mini-btn ${state.theme === 'dark' ? 'buy' : ''}" onclick="changeTheme('dark');this.closest('.modal-overlay').remove();openPreferencesModal();"><img class="ui-icon" src="${ICONS.moon}" alt="🌙"> Dark</button>
        <button class="mini-btn ${state.theme === 'light' ? 'buy' : ''}" onclick="changeTheme('light');this.closest('.modal-overlay').remove();openPreferencesModal();"><img class="ui-icon" src="${ICONS.sun}" alt="☀"> Light</button>
    `;
    ...
    const body = `
        ...
        <div style="margin-bottom:16px;">
            <div style="font-family:'Cairo',sans-serif;font-weight:700;font-size:13px;color:var(--brass-bright);margin-bottom:8px;"><img class="ui-icon" src="${ICONS.palette}" alt="🎨"> Theme</div>
    ...
    `;
    showModal(`<img class="ui-icon" src="${ICONS.palette}" alt="🎨"> Appearance`, body);
}

function openAchievementsModal() {
    const body = `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;font-size:12.5px;color:var(--dim);">
            <div><img class="ui-icon" src="${ICONS.gold_coin}" alt="🪙"> Gold: <b style="color:var(--text);">${fmtG(state.gold)}</b></div>
            <div><img class="ui-icon" src="${ICONS.medal_sport}" alt="🏅"> Level: <b style="color:var(--text);">${state.level}</b></div>
            <div><img class="ui-icon" src="${ICONS.damage_ui}" alt="⚔"> Wins: <b style="color:var(--green);">${state.combat.wins}</b></div>
            <div><img class="ui-icon" src="${ICONS.skull}" alt="💀"> Losses: <b style="color:var(--red);">${state.combat.losses}</b></div>
            <div><img class="ui-icon" src="${ICONS.bag_full}" alt="🎒"> Gear Items: <b style="color:var(--text);">${state.gearBag.length}</b></div>
            <div><img class="ui-icon" src="${ICONS.shard}" alt="🔷"> Shards: <b style="color:var(--text);">${state.shards}</b></div>
            <div><img class="ui-icon" src="${ICONS.gem}" alt="💎"> Gems: <b style="color:var(--text);">${state.gems}</b></div>
            <div><img class="ui-icon" src="${ICONS.spellbook}" alt="🎯"> Skill Points: <b style="color:var(--text);">${state.skillPoints}</b></div>
            <div><img class="ui-icon" src="${ICONS.business}" alt="💰"> Total Gold Earned: <b style="color:var(--text);">${fmtG(state.totalGoldEarned)}</b></div>
            <div><img class="ui-icon" src="${ICONS.handshake}" alt="🤝"> Total Donated to Alliances: <b style="color:var(--text);">${fmtG(state.totalAllianceDonated || 0)}</b></div>
            <div><img class="ui-icon" src="${ICONS.sparkle}" alt="✨"> Prestige Points: <b style="color:var(--prestige);">${state.prestige.points}</b></div>
        </div>
    `;
    showModal(`<img class="ui-icon" src="${ICONS.trophy}" alt="🏆"> Stats & Achievements`, body);
}
...
function openSecurityModal() {
    ...
    const body = isGuest ? `
        ...
        <button class="act-btn buy" style="width:auto;padding:8px 16px;font-size:12px;" onclick="linkGoogleAccount()"><img class="ui-icon" src="${ICONS.link}" alt="🔗"> Link Google Account</button>
    ` : `
        <p style="font-size:12.5px;color:var(--dim);margin-bottom:14px;">Account linked to <b style="color:var(--text);">${EMAIL}</b></p>
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
            <button class="act-btn skill" style="width:auto;padding:8px 16px;font-size:12px;" onclick="changePassword()"><img class="ui-icon" src="${ICONS.lock}" alt="🔒"> Reset Password</button>
            <button class="act-btn red" style="width:auto;padding:8px 16px;font-size:12px;" onclick="if(confirm('Delete your account permanently?'))deleteAccount()"><img class="ui-icon" src="${ICONS.trash}" alt="🗑"> Delete Account</button>
        </div>
    `;
    showModal(`<img class="ui-icon" src="${ICONS.lock_key}" alt="🔐"> Login & Security`, body);
}
...
function openDataModal() {
    const body = `
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
            <button class="mini-btn buy" onclick="exportData()"><img class="ui-icon" src="${ICONS.inbox}" alt="📥"> Export Data</button>
            <button class="mini-btn" onclick="document.getElementById('importInput').click()"><img class="ui-icon" src="${ICONS.outbox}" alt="📤"> Import Data</button>
            <input type="file" id="importInput" accept=".json" style="display:none;" onchange="importData(this.files[0])">
            <button class="mini-btn red" onclick="if(confirm('Clear all local data?'))clearLocalData()"><img class="ui-icon" src="${ICONS.trash}" alt="🗑"> Clear Local</button>
        </div>
    `;
    showModal(`<img class="ui-icon" src="${ICONS.market_chart}" alt="📊"> Data & Backup`, body);
}
...
function openPrestigeModal() {
    ...
    const body = `
        <div style="text-align:center;">
            <div style="font-size:36px;margin-bottom:6px;"><img class="ui-icon" src="${ICONS.sparkle}" alt="✨"></div>
            ...
            ${canPrest ? `<div style="color:var(--prestige);margin:8px 0;font-size:13px;"><img class="ui-icon" src="${ICONS.party}" alt="🎉"> You will gain <b>${Math.floor(state.level/5) + Math.floor(state.totalGoldEarned/5000)}</b> prestige points!</div>` : ''}
            ...
            <button class="act-btn prestige ${canPrest?'animate-pulse':''}" style="width:auto;padding:12px 28px;font-size:13px;" ${canPrest?'':'disabled'} onclick="doPrestige();this.closest('.modal-overlay').remove();">
                ${canPrest ? `<img class="ui-icon" src="${ICONS.sparkle}" alt="✨"> Spiritual Renewal` : `<img class="ui-icon" src="${ICONS.lock}" alt="🔒"> Requires level ${PRESTIGE_LEVEL_REQ}`}
            </button>
        </div>
    `;
    showModal(`<img class="ui-icon" src="${ICONS.sparkle}" alt="✨"> Prestige — Spiritual Renewal`, body);
}
    ...
    const body = `
        <div style="font-size:12.5px;color:var(--dim);">
            ...
            <div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap;">
                <button class="mini-btn" onclick="alert('Report a bug: arcadia@email.com')"><img class="ui-icon" src="${ICONS.bug}" alt="🐛"> Report Bug</button>
                <button class="mini-btn buy" onclick="alert('Send feedback: arcadia@email.com')"><img class="ui-icon" src="${ICONS.lightbulb}" alt="💡"> Feedback</button>
            </div>
        </div>
    `;
...
function openNameModal() {
    ...
    modal.innerHTML = `
        <div class="modal-box">
            <div class="modal-header">
                <h3><img class="ui-icon" src="${ICONS.pencil}" alt="✏"> Change Username</h3>
                <button class="modal-close" onclick="this.closest('.modal-overlay').remove()"><img class="ui-icon" src="${ICONS.close}" alt="✕"></button>
            </div>
            <div class="modal-body">
                ...
                <div id="nameSuccess" style="color:var(--green);font-size:12px;display:none;"><img class="ui-icon" src="${ICONS.check}" alt="✓"> Username available!</div>
        ...
        nameCheckTimeout = setTimeout(async () => {
            try {
                if (db) {
                    ...
                    if (doc.exists) {
                        errorEl.innerHTML = `<img class="ui-icon" src="${ICONS.cancel}" alt="❌"> Username already taken`;
                        errorEl.style.display = 'block';
                        return;
                    }
                }
...
// Like hmsUntil but switches to a "Xd Yh" form once the countdown crosses
// a day — used by weekly/monthly timers where raw hours would run into the
// hundreds and stop being readable at a glance.
...
function renderZonesTab(){
  if(typeof zoneSubTab === 'undefined' || !territoryFeatureActive()){
    return state.zoneView ? renderZoneView() : renderZones();
  }
  const subTabs = [
    { id:'adventure', label:`<img class="ui-icon" src="${ICONS.zones_map}" alt="🗺"> Explore` },
    { id:'territory',   label:`<img class="ui-icon" src="${ICONS.chess_queen}" alt="♛"> Kingdom Map` },
  ];
  const toggleBar = `<div class="wars-filter-tabs" style="margin-bottom:10px;">${subTabs.map(tb =>
    `<button class="wars-filter-btn ${zoneSubTab === tb.id ? 'active' : ''}" onclick="setZoneSubTab('${tb.id}')">${tb.label}</button>`
  ).join('')}</div>`;
  let body;
  if(zoneSubTab === 'territory') body = renderKingdomMap();
  else body = state.zoneView ? renderZoneView() : renderZones();
  return `${toggleBar}${body}`;
}
// Kingdom Map / Active Wars only make sense once territory.js (and its d3/topojson
// dependency) is actually loaded — falls back to plain zone exploration otherwise.
function territoryFeatureActive(){
  return typeof loadTerritories === 'function';
}
        ...
        return `<div onclick="${locked?'':'enterZoneView(\''+z.id+'\')'}" 
          ...
          <div style="padding:14px;">
            ...
            <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px;">
              ...
              <span style="background:var(--panel-inset);border:1px solid var(--border-light);border-radius:6px;padding:3px 8px;font-family:'JetBrains Mono',monospace;font-size:10px;color:var(--dim);"><img class="ui-icon" src="${ICONS.paw}" alt="🐾"> ${monsterCount}</span>
            </div>
            ${locked
              ? `<div style="background:var(--panel-inset);border-radius:6px;padding:8px;text-align:center;font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--copper);"><img class="ui-icon" src="${ICONS.lock}" alt="🔒"> Requires Lv.${z.levelMin}</div>`
...
function renderZoneView(){
  ...
  return `<div class="wrap animate-fade">
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;">
      <button class="act-btn" style="width:auto;padding:6px 12px;font-size:11px;" onclick="goBackFromZone()"><img class="ui-icon" src="${ICONS.arrow_left}" alt="←"> Back to Zones</button>
    ...
    <div style="font-family:'Cairo',sans-serif;font-weight:700;font-size:14px;color:var(--green);margin-bottom:8px;"><img class="ui-icon" src="${ICONS.craft}" alt="⚒"> Resources</div>
    <div class="grid" style="grid-template-columns:repeat(auto-fill,minmax(160px,1fr));margin-bottom:20px;">
      ${resources.map(rid=>{
        ...
        return `<div class="card" style="padding:14px;text-align:center;">
          ...
          <button class="act-btn green" onclick="collect('${rid}')" ${canGather?'':'disabled'}>Gather (-5<img class="ui-icon" src="${ICONS.energy}" alt="⚡">)</button>
        </div>`;
      }).join('')}
    </div>

    <div style="font-family:'Cairo',sans-serif;font-weight:700;font-size:14px;color:var(--red);margin-bottom:8px;"><img class="ui-icon" src="${ICONS.damage_ui}" alt="⚔"> Monsters</div>
...
function ppRankForLevel(lvl){
  if(lvl >= 50) return { name:'Legend', icon:`<img class="ui-icon" src="${ICONS.crown}" alt="👑">` };
  if(lvl >= 35) return { name:'Champion', icon:`<img class="ui-icon" src="${ICONS.trophy}" alt="🏆">` };
  if(lvl >= 20) return { name:'Veteran', icon:`<img class="ui-icon" src="${ICONS.medal}" alt="🎖">` };
  if(lvl >= 10) return { name:'Adventurer', icon:`<img class="ui-icon" src="${ICONS.compass}" alt="🧭">` };
  return { name:'Novice', icon:`<img class="ui-icon" src="${ICONS.seedling}" alt="🌱">` };
}

function buildPlayerProfileHtml(p, isSelf){
  const cls = p.playerClass ? CLASS_DATA[p.playerClass] : null;
  const clsColor = cls ? cls.color : '#d4a24c';
  const clsName = escapeHtml(cls ? cls.nameAr : 'Adventurer');
  const clsIcon = cls ? cls.icon : `<img class="ui-icon" src="${ICONS.compass}" alt="🧭">`;
  const rank = ppRankForLevel(p.level || 1);

  const totalGames = p.wins + p.losses;
  const winPct = totalGames > 0 ? Math.round(p.wins/totalGames*100) : 0;
  // Circle ring math (r=42, circumference ≈ 264)
  const ppRingR = 42;
  const ppRingC = Math.round(2 * Math.PI * ppRingR);
  const ppRingOffset = Math.round(ppRingC * (1 - winPct/100));

  const allianceHtml = p.allianceName ? `
    <div class="pp-alliance">
      <div class="pp-alliance-icon"><img class="ui-icon" src="${ICONS.alliance}" alt="🏛"></div>
      <div>
        <div class="pp-alliance-name">${escapeHtml(p.allianceName)}</div>
        <div class="pp-alliance-role">${escapeHtml(p.allianceRole || 'Member')}</div>
      </div>
      <div class="pp-alliance-chevron">›</div>
    </div>
  ` : '';

  const bioHtml = p.bio ? `
    <div class="pp-bio">
      <span class="pp-bio-mark">“</span>${escapeHtml(p.bio)}<span class="pp-bio-mark">”</span>
    </div>
  ` : '';

  const medallions = [
    { icon:`<img class="ui-icon" src="${ICONS.gold_coin}" alt="🪙">`, val:fmtG(p.gold), label:'Gold', color:'var(--brass-bright)' },
    { icon:`<img class="ui-icon" src="${ICONS.damage_ui}" alt="⚔">`, val:p.wins, label:'Wins', color:'var(--green)' },
    { icon:`<img class="ui-icon" src="${ICONS.skull}" alt="💀">`, val:p.losses, label:'Losses', color:'var(--red)' },
    { icon:`<img class="ui-icon" src="${ICONS.sparkle}" alt="✨">`, val:p.prestigePoints || 0, label:'Prestige', color:'var(--prestige)' },
    { icon:`<img class="ui-icon" src="${ICONS.business}" alt="💰">`, val:fmtG(p.totalGoldEarned || p.gold), label:'Earned', color:'var(--brass-bright)' },
    { icon:`<img class="ui-icon" src="${ICONS.bag_full}" alt="🎒">`, val:p.gearCount || 0, label:'Gear', color:'var(--skill)' },
  ].map(m => `
    <div class="pp-medallion">
      <div class="pp-medallion-badge" style="--m:${m.color};"><span>${m.icon}</span></div>
      <div class="pp-medallion-val" style="color:${m.color};">${m.val}</div>
      <div class="pp-medallion-label">${m.label}</div>
    </div>
  `).join('');

  return `
    <div class="pp-container" style="--cls:${clsColor};">
      <!-- Banner -->
      <div class="pp-banner">
        <div class="pp-banner-glow"></div>
        <div class="pp-rank-ribbon"><span>${rank.icon}</span>${rank.name}</div>

        <div class="pp-hex-wrap">
          <div class="pp-hex-ring">
            <div class="pp-hex-avatar">${escapeHtml(p.avatar)}</div>
          </div>
          <div class="pp-hex-level">${p.level}</div>
        </div>

        <div class="pp-name">${escapeHtml(p.username)}</div>
        <div class="pp-class-tag" style="--cc:${clsColor};">${clsIcon} ${clsName}</div>
      </div>
      ...
      <!-- Medallion Stats -->
      <div class="pp-medallion-grid">${medallions}</div>

      <!-- Win Rate Ring -->
      <div class="pp-ring-card">
        <div class="pp-ring-svg-wrap">
          <svg viewBox="0 0 100 100" class="pp-ring-svg">
            <circle cx="50" cy="50" r="${ppRingR}" class="pp-ring-track"/>
            <circle cx="50" cy="50" r="${ppRingR}" class="pp-ring-fill"
              style="stroke-dasharray:${ppRingC};stroke-dashoffset:${ppRingOffset};"/>
          </svg>
          <div class="pp-ring-center">
            <div class="pp-ring-pct">${winPct}%</div>
            <div class="pp-ring-sub">Win Rate</div>
          </div>
        </div>
        <div class="pp-ring-side">
          <div class="pp-ring-row"><span class="dot" style="background:var(--green);"></span>Wins<b>${p.wins}</b></div>
          <div class="pp-ring-row"><span class="dot" style="background:var(--red);"></span>Losses<b>${p.losses}</b></div>
          <div class="pp-ring-row"><span class="dot" style="background:var(--dim);"></span>Total<b>${totalGames}</b></div>
        </div>
      </div>

      ${isSelf ? '' : `
        <div style="display:flex;gap:8px;margin-top:14px;">
          <button class="act-btn" style="flex:1;" onclick="showToast('Coming Soon','Private messaging will be available soon!','win')"><img class="ui-icon" src="${ICONS.chat}" alt="💬"> Message</button>
        </div>
      `}
    </div>
  `;
...
function showPlayerProfile(playerData){
  ...
  // wins/losses here are PvP-only (state.pvp), not monster combat (state.combat)
  ...
  const p = playerData || {
    ...
    wins: (state.pvp && state.pvp.wins) || 0,
    losses: (state.pvp && state.pvp.losses) || 0,
    gold: state.gold,
  ...
  const body = buildPlayerProfileHtml(p, isSelf);
  showModal(isSelf ? `<img class="ui-icon" src="${ICONS.player_icon}" alt="👤"> Your Profile` : `<img class="ui-icon" src="${ICONS.player_icon}" alt="👤"> ${escapeHtml(p.username)}`, body);
}
