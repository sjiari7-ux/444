...
function renderMissions(){
  ...
  const tabs = [
    ...
    { id:'monthly', label:'Monthly', icon:'🌙', data: mData.monthly, pool: MONTHLY_MISSIONS },
  ];
...
function ppRankForLevel(lvl){
  if(lvl >= 50) return { name:'Legend', icon:'👑' };
  if(lvl >= 35) return { name:'Champion', icon:`<img class="ui-icon" src="${ICONS.trophy}" alt="🏆">` };
  if(lvl >= 20) return { name:'Veteran', icon:'🎖️' };
  if(lvl >= 10) return { name:'Adventurer', icon:'🧭' };
...
function buildPlayerProfileHtml(p, isSelf){
  ...
  const medallions = [
    ...
    { icon:'💀', val:p.losses, label:'Losses', color:'var(--red)' },
    { icon:'✨', val:p.prestigePoints || 0, label:'Prestige', color:'var(--prestige)' },
...
function showPlayerProfile(playerData){
  ...
  showModal(isSelf ? '👤 Your Profile' : `👤 ${escapeHtml(p.username)}`, body);
}
