...
function renderLeaderboard(){
  ...
  return `
    ...
    <div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(240px,1fr));">
      ...
      <div class="panel"><div class="section-title" style="margin-top:0;"><h2><img class="ui-icon" src="${ICONS.damage_ui}" alt="⚔"> Top PvP Rating</h2><span style="cursor:pointer;font-size:11px;color:var(--brass-bright);" onclick="openPvpRankTop100()">Top 100 →</span></div>${tbl(leaderboardByPvpRating||[],'pvpRating')}</div>
...
function renderKingdomLeaderboardPanel(){
  ...
  const rows = list.map((k, i) => `
    <tr class="${k.mine ? 'me' : ''}" style="cursor:pointer;" onclick="focusKingdomOnWorldMap('${k.id}')" title="View on the world map">
      <td class="lb-rank">${i+1}</td>
      <td><span class="war-fighter-emblem" style="background:${k.color}">${k.emblem}</span> ${escapeHtml(k.name)}</td>
      <td>${k.territories}/30</td>
      <td>${k.warsWon}-${k.warsLost}</td>
      <td>${formatWarNumber(k.totalWarDamage)}</td>
      <td>${k.memberCount}</td>
    </tr>`).join('');
  return `
    <div class="panel" style="margin-bottom:14px;">
      ...
      <table class="lb-table">
        <tr><th>#</th><th>Kingdom</th><th>Territories</th><th>Sieges W-L</th><th>War Dmg</th><th>Citizens</th></tr>
        ${rows || `<tr><td colspan="6" class="lb-note">No kingdoms yet.</td></tr>`}
      </table>
    </div>`;
}
