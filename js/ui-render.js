...
function renderMarket(){
  ...
  const view = state.marketView || 'browse';
  const body = view === 'mine' ? renderMyListingsView() : renderMarketBrowseView();
  return `
    <div class="panel" style="padding:0;overflow:hidden;">
      <div style="display:flex;justify-content:space-between;align-items:center;padding:12px 16px;border-bottom:1px solid var(--border);background:var(--panel-light);flex-wrap:wrap;gap:8px;">
        ...
        <div style="font-size:11px;color:var(--dim);">Every price here is a real player's listing — no auto-generated prices.</div>
      </div>
      <div style="display:flex;gap:8px;padding:12px 16px;border-bottom:1px solid var(--border);background:var(--panel);">
        <button class="market-tab-btn ${view==='browse'?'active':''}" onclick="setMarketView('browse')">🛒 Browse</button>
        <button class="market-tab-btn ${view==='mine'?'active':''}" onclick="setMarketView('mine')">🏷️ My Listings</button>
      </div>
      ${body}
    </div>
