...
function renderBodyHTML(){
  ...
  switch(activeTab){
    ...
    // V1: 'battles' has no nav entry point anymore (Kingdom Wars deferred —
    // see V1_CHANGES.md), but this guard stays so nothing throws if
    // activeTab is ever set to 'battles' some other way.
    case 'battles': html = (typeof renderActiveWarsList === 'function') ? renderActiveWarsList() : ''; break;
    default: html = ''; break;
  }
...
function renderMarket(){
  ...
  // V1: Gear Market deferred (see V1_CHANGES.md) — category is locked to
  // 'resources'. renderGearMarketSection() and setMarketCategory('gear')
  // still work; just restore the category toggle buttons below to reopen it.
  const category = 'resources';
  const isGear = false;
  const view = isGear ? (state.gearMarketView || 'browse') : (state.marketView || 'browse');
  const subtext = "Every price here is a real player's listing — no auto-generated prices.";
...
// V1: leaderboard narrowed to PvP Ranking only (see V1_CHANGES.md). The
// Wealth/Level/Kingdoms views still work underneath — just uncomment the
// tabs bar below and the metric switcher in renderCitizenRankings() to
// bring them back once there's a real population to rank.
function renderLeaderboard(){
  ...
  citizenLbMetric = 'pvpRating';
  return renderCitizenRankings();
}
...
function renderCitizenRankings(){
  ...
  // V1: metric locked to PvP Rating, so these dropdowns don't render — the
  // underlying setCitizenLbMetric/setCitizenLbKingdom functions are untouched.
  const metricMenu = '';
  const kingdomMenu = '';
  ...
  return `
    <div class="lb-selectors">
      <div style="position:relative;flex:1;">
        <div class="lb-select-btn" style="cursor:default;opacity:.85;"><span><img class="ui-icon" src="${metric.icon()}" alt=""> ${metric.label}</span></div>
      </div>
    </div>
    ${tierStrip}
    ${myRankStrip}
