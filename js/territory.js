...
async function contributeToWar(tid, side){
  ...
  try{
    ...
    db.collection('alliances').doc(state.allianceId).set({
      totalWarDamage: firebase.firestore.FieldValue.increment(dmg),
    }, { merge: true }).catch(e => console.error('[Arcadia Territory] Kingdom damage tally failed:', e.code || e.name, e.message));
    ...
    showToast(isAttacker ? `<img class="ui-icon" src="${ICONS.damage_ui}" alt="⚔">` : `<img class="ui-icon" src="${ICONS.defense_ui}" alt="🛡">`, isAttacker ? 'Strike!' : 'Defended!', `+${dmg} damage this round`);
  }catch(e){
    state.energy += cost;
...
/* Tally a finished siege onto both kingdoms' alliance docs, for the Kingdoms leaderboard.
   Uses set(merge:true) instead of update() because the losing/defending kingdom may not
   have an alliance doc yet if nobody has pledged to it. */
async function creditKingdomWarResult(attackerKingdom, defenderKingdom, winner){
  if(!db || !attackerKingdom || !defenderKingdom) return;
  const attackerWon = winner === 'attacker';
  try{
    await Promise.all([
      db.collection('alliances').doc(attackerKingdom).set({
        warsWon: firebase.firestore.FieldValue.increment(attackerWon ? 1 : 0),
        warsLost: firebase.firestore.FieldValue.increment(attackerWon ? 0 : 1),
      }, { merge: true }),
      db.collection('alliances').doc(defenderKingdom).set({
        warsWon: firebase.firestore.FieldValue.increment(attackerWon ? 0 : 1),
        warsLost: firebase.firestore.FieldValue.increment(attackerWon ? 1 : 0),
      }, { merge: true }),
    ]);
  }catch(e){ console.error('[Arcadia Territory] Kingdom war credit failed:', e.code || e.name, e.message); }
}
...
async function resolveWarRoundIfDue(tid){
  ...
  try{
    ...
    outcome = await db.runTransaction(async (tx) => {
      ...
      if(attackerWins >= WAR_ROUNDS_TO_WIN){
        ...
        return { finished: true, winner: 'attacker', attackerKingdom: war.attackerKingdom, defenderKingdom: cur.ownerKingdom };
      }
      if(defenderWins >= WAR_ROUNDS_TO_WIN){
        ...
        return { finished: true, winner: 'defender', attackerKingdom: war.attackerKingdom, defenderKingdom: cur.ownerKingdom };
      }
      tx.update(ref, {
  ...
  if(!outcome) return;
  if(outcome.finished) await creditKingdomWarResult(outcome.attackerKingdom, outcome.defenderKingdom, outcome.winner);
  await refreshTerritoryDoc(tid);
  if(outcome.finished && outcome.winner === 'attacker') showToast(`<img class="ui-icon" src="${ICONS.flag_black}" alt="🏴">`, 'War Won!', `${tid.toUpperCase()} has fallen!`);
  else if(outcome.finished && outcome.winner === 'defender') showToast(`<img class="ui-icon" src="${ICONS.defense_ui}" alt="🛡">`, 'Siege Repelled', `${tid.toUpperCase()} held strong.`);
  else if(outcome.finished === false) showToast(`<img class="ui-icon" src="${ICONS.damage_ui}" alt="⚔">`, 'Round Over', `Round ${outcome.nextRound} of 3 begins.`);
}
...
function renderKingdomMapSVG(myKingdom){
  ...
  const svgTerritories=territories.map(o=>{
    ...
    return `<g class="world-territory ${selected?'selected':''}" data-territory="${o.meta.id}">
      ...
      ${o.idx===0?'<text class="capital-crown" x="'+cx+'" y="'+(cy-22)+`"><img class="ui-icon" src="${ICONS.chess_queen}" alt="♛"></text>`:''}
    ...
    <div id="arcadia-map-viewport" class="arcadia-map-viewport">
      <svg viewBox="0 0 1040 700" aria-label="ARCADIA World Map">
        <defs>
          ...
          <radialGradient id="sea" cx="50%" cy="38%" r="75%"><stop offset="0%" stop-color="#141a20"/><stop offset="55%" stop-color="#0a0d12"/><stop offset="100%" stop-color="#04060a"/></radialGradient>
        ...
        <g opacity=".14" stroke="#8a3a1c"><path d="M0 140H1040M0 280H1040M0 420H1040M0 560H1040"/><path d="M130 0V700M260 0V700M390 0V700M520 0V700M650 0V700M780 0V700M910 0V700"/></g>
      ...
      <div class="map-legend"><b>LEGEND</b><span><i class="legend-swatch own"></i>Your kingdom</span><span><i class="legend-swatch"></i>Other kingdom</span><span><img class="ui-icon" src="${ICONS.chess_queen}" alt="♛"> Capital</span></div>
    </div>
...
function renderWarDetailModal(){
  ...
  return `<div class="war-modal-backdrop" onclick="if(event.target===this)closeWarModal()">
    <div class="war-modal">
      <button class="war-modal-close" onclick="closeWarModal()"><img class="ui-icon" src="${ICONS.close}" alt="✕"></button>
