...
/* ═══════════════════════════════════════════════════════════════
   WORLD_CONFIG — single source of truth for every kingdom/zone.
   Before this, one kingdom's data was spread across five places:
   ARCADIA_WORLD (names/label/color, further down this file),
   ZONE_ADJACENCY, ZONE_HOME_KINGDOM (both used to live right here),
   ZONE_TAX_RATE, and ZONE_RESOURCES (used to live in state.js) —
   changing one kingdom meant editing several spots and hoping
   nothing was missed. All of those are now DERIVED from this one
   table below, so adding or adjusting a kingdom is a one-place edit.
   Region count per zone is regionNames.length. All six still have 5
   (Capital + Outpost/Fortress Gate/Mountain Pass + West — see the
   Ancient Core route logic further down), but nothing here forces
   that anymore — a zone with a different regionNames length is a
   config edit, not a rewrite. (The Core-route indices themselves
   are still fixed at 2/3/4 and assume 5 regions; giving those their
   own per-zone config is the natural next step once a zone actually
   needs a different length.) */
const WORLD_CONFIG = {
  plains: {
    kingdomId: 'europe', label: 'Europe', color: '#3b6cb5',
    resources: ['cotton', 'food', 'herbs', 'water'],
    regionNames: ['European Capital', 'Northern Europe', 'Eastern Europe', 'Southern Europe', 'Western Europe'],
    adjacentZones: ['forest', 'mountain', 'swamp'],
    taxRate: 0.05,
  },
  forest: {
    kingdomId: 'asia', label: 'Asia', color: '#d4a843',
    resources: ['wood', 'herbs', 'food', 'water'],
    regionNames: ['Asian Capital', 'Northern Asia', 'Eastern Asia', 'Southern Asia', 'Western Asia'],
    adjacentZones: ['plains', 'cave'],
    taxRate: 0.08,
  },
  mountain: {
    kingdomId: 'africa', label: 'Africa', color: '#4a9b5e',
    resources: ['stone', 'iron', 'coal', 'zinc'],
    regionNames: ['African Capital', 'Northern Africa', 'Eastern Africa', 'Southern Africa', 'Western Africa'],
    adjacentZones: ['swamp', 'plains'],
    taxRate: 0.10,
  },
  cave: {
    kingdomId: 'north_america', label: 'North America', color: '#b03a2e',
    resources: ['stone', 'iron', 'coal', 'silver'],
    regionNames: ['NA Capital', 'Northern NA', 'Eastern NA', 'Southern NA', 'Western NA'],
    adjacentZones: ['forest', 'dark'],
    taxRate: 0.13,
  },
  swamp: {
    kingdomId: 'arab_world', label: 'Arab World', color: '#c17f24',
    resources: ['herbs', 'water', 'leather', 'salt'],
    regionNames: ['Arab Capital', 'Northern Arabia', 'Eastern Arabia', 'Southern Arabia', 'Western Arabia'],
    adjacentZones: ['dark', 'mountain', 'plains'],
    taxRate: 0.16,
  },
  dark: {
    kingdomId: 'south_america', label: 'South America', color: '#7d3c98',
    resources: ['magic_stones', 'gemstones', 'gold', 'lead'],
    regionNames: ['SA Capital', 'Northern SA', 'Eastern SA', 'Southern SA', 'Western SA'],
    adjacentZones: ['cave', 'swamp'],
    taxRate: 0.20,
  },
};

/* ===== DERIVED FROM WORLD_CONFIG — edit the table above, not these ===== */
const ZONE_ADJACENCY = {};
const ZONE_HOME_KINGDOM = {};
const ZONE_TAX_RATE = {};
const ZONE_RESOURCES = {};
Object.keys(WORLD_CONFIG).forEach(z => {
  const cfg = WORLD_CONFIG[z];
  ZONE_ADJACENCY[z] = cfg.adjacentZones;
  ZONE_HOME_KINGDOM[z] = cfg.kingdomId;
  ZONE_TAX_RATE[z] = cfg.taxRate;
  ZONE_RESOURCES[z] = cfg.resources;
});
const KINGDOM_HOME_ZONE = {};
Object.keys(ZONE_HOME_KINGDOM).forEach(z => { KINGDOM_HOME_ZONE[ZONE_HOME_KINGDOM[z]] = z; });
function zoneRegionCount(zoneId){
  return (WORLD_CONFIG[zoneId] && WORLD_CONFIG[zoneId].regionNames.length) || TERRITORIES_PER_ZONE;
}
// Majority needed to hold zone-wide tax rights: "more than half" of that
// zone's OWN region count, not a single global number — so a bigger zone
// (once one exists) needs a bigger majority, not the same fixed 3.
function zoneMajorityThreshold(zoneId){
  return Math.ceil((zoneRegionCount(zoneId) + 1) / 2);
}

/* ===== CONFIG =====
   TERRITORIES_PER_ZONE / ZONE_MAJORITY_THRESHOLD below are now only the
   FALLBACK default (used by zoneRegionCount when a zone id isn't in
   WORLD_CONFIG) — actual game logic reads zoneRegionCount(zone) /
   zoneMajorityThreshold(zone) so each zone can have its own region
   count. Don't reintroduce a global TERRITORIES_PER_ZONE read in new
   code; add a per-zone-aware call instead. */
...
/* ===== ANCIENT CORE — ROUTE GRAPH =====
   ...
   The Gate stays pointing at the *middle* route link specifically — it's
   the one link that's also independently meaningful (the map badge, and
   historically the sole check before this route chain existed).
   ...
   All of this is now derived PER ZONE from zoneRegionCount(zoneId) instead
   of a single global 5-region assumption: the route is every non-capital
   index except the last one (the "West" slot stays outside the route,
   same as before), and the gate is the middle of that route. For today's
   six zones (all 5 regions) this reproduces the exact same numbers as the
   old hardcoded CORE_GATE_INDEX=3 / CORE_ROUTE_INDICES=[2,3,4] — a zone
   with a different regionNames length in WORLD_CONFIG now gets a correctly
   sized route automatically instead of needing this section rewritten. */
function zoneCoreRouteIndices(zoneId){
  const n = zoneRegionCount(zoneId);
  const indices = [];
  for(let i = TERRITORY_CAPITAL_INDEX + 1; i <= n - 1; i++) indices.push(i);
  return indices.length ? indices : [TERRITORY_CAPITAL_INDEX + 1];
}
function zoneCoreGateIndex(zoneId){
  const route = zoneCoreRouteIndices(zoneId);
  return route[Math.floor((route.length - 1) / 2)];
}
// Labels for the route: the exact wording used to match the old fixed
// 3-link route (Outpost/Fortress Gate/Mountain Pass) for every zone that
// still has that shape; falls back to generic Outpost/Waypoint/Mountain
// Pass naming for a future zone whose route is a different length.
function coreRouteLabelFor(zoneId, idx){
  const route = zoneCoreRouteIndices(zoneId);
  const pos = route.indexOf(idx);
  if(pos === -1) return null;
  if(route.length === 3) return ['Outpost', 'Fortress Gate', 'Mountain Pass'][pos];
  if(pos === 0) return 'Outpost';
  if(pos === route.length - 1) return 'Mountain Pass';
  return `Waypoint ${pos}`;
}
function tidZoneAndIdx(tid){
  const parts = String(tid).split('_');
  return { zone: parts[0], idx: parseInt(parts[1], 10) };
}
function coreGateId(zone){ return territoryDocId(zone, zoneCoreGateIndex(zone)); }
function isCoreGateTerritory(tid){
  const { zone, idx } = tidZoneAndIdx(tid);
  return idx === zoneCoreGateIndex(zone);
}
function territoryRouteLabel(tid){
  const { zone, idx } = tidZoneAndIdx(tid);
  return coreRouteLabelFor(zone, idx);
}
...
function kingdomHoldsCoreGate(kingdomId){
  ...
  return zoneCoreRouteIndices(homeZone).every(idx => {
...
function coreRouteChainForKingdom(kingdomId){
  ...
  zoneCoreRouteIndices(homeZone).forEach(idx => {
    nodes.push({ idx, id: territoryDocId(homeZone, idx), label: coreRouteLabelFor(homeZone, idx) || `Outpost ${idx}`, zone: homeZone });
  });
  return nodes;
}
...
// Same "server-only doc, client just reads" pattern as coreData above —
// meta/season is bootstrapped and rolled over exclusively by Cloud
// Functions (ensureSeasonDocExists/resolveDueSeason in functions/index.js).
let seasonData = null;
let seasonLoaded = false;
let seasonLoading = false;
let activeSeasonHistoryOpen = false;
...
async function ensureTerritoriesSeeded(){
  ...
  try{
    ...
    Object.keys(ZONE_HOME_KINGDOM).forEach(zone => {
      ...
      const regionCount = zoneRegionCount(zone);
    ...
    await batch.commit();
    console.log('[Arcadia Territory] Seeded', seededCount, 'outposts across', Object.keys(ZONE_HOME_KINGDOM).length, 'zones');
  }catch(e){
...
async function loadSeason(force){
  if(!db){ seasonLoaded = true; return; }
  if(seasonLoading) return;
  if(seasonLoaded && !force) return;
  seasonLoading = true;
  try{
    const doc = await db.collection('meta').doc('season').get();
    seasonData = doc.exists ? { id: doc.id, ...doc.data() } : null;
    seasonLoaded = true;
  }catch(e){
    console.error('[Arcadia Season] Load failed:', e.code || e.name, e.message);
    seasonLoaded = true;
  }
  seasonLoading = false;
  if(typeof renderBody === 'function' && activeTab === 'battles') renderBody();
}

async function initTerritoryOnStart(){
  if(!db) { territoryLoaded = true; worldGeometryLoaded = true; coreLoaded = true; seasonLoaded = true; return; }
  bindWarTicker();
  await Promise.all([
...
function zoneTaxOwner(zone){
  ...
  return topCount >= zoneMajorityThreshold(zone) ? topKingdom : null;
}

/* ===== TERRITORY SPECIALTIES — regional resource deposits =====
   Each zone's 4 non-capital outposts (Northern/Eastern/Southern/Western)
   carries one of that zone's 4 native resources (ZONE_RESOURCES, state.js)
   as its "deposit" — one deposit per outpost, same order as ZONE_RESOURCES.
   This mirrors WarEra's per-region resource deposits: it gives the map
   real economic weight, so *which* outpost a kingdom holds — not just how
   many — matters. Holding a deposit raises the kingdom's tax take on that
   one matching resource anywhere in the zone (TERRITORY_SPECIALTY_TAX_BONUS
   per deposit held), on top of the flat ZONE_TAX_RATE. */
const TERRITORY_SPECIALTY_TAX_BONUS = 0.5; // +50% tax rate on the matching resource, per deposit held
function territorySpecialty(zoneId, rawIdx){
  if(rawIdx === TERRITORY_CAPITAL_INDEX) return null;
  const pool = (typeof ZONE_RESOURCES !== 'undefined' && ZONE_RESOURCES[zoneId]) || null;
  if(!pool || !pool.length) return null;
  return pool[(rawIdx - TERRITORY_CAPITAL_INDEX - 1) % pool.length];
}
function kingdomSpecialtyBonus(kingdomId, zoneId, resourceKey){
  if(!kingdomId || !resourceKey) return 0;
  let bonus = 0;
  for(let rawIdx = 1; rawIdx <= zoneRegionCount(zoneId); rawIdx++){
    const t = territoryData[territoryDocId(zoneId, rawIdx)];
    if(t && t.ownerKingdom === kingdomId && territorySpecialty(zoneId, rawIdx) === resourceKey){
      bonus += TERRITORY_SPECIALTY_TAX_BONUS;
    }
  }
  return bonus;
}
...
function applyZoneTax(zoneId, resourceKey, grossAmount){
  ...
  const rate = (ZONE_TAX_RATE[zoneId] || 0) * (1 + kingdomSpecialtyBonus(owner, zoneId, resourceKey));
...
// Commander: Command Points earned in regular battles pay off here — spend
// some to hit harder on a single war/Core contribution. Shared by
// contributeToWar and contributeToCore below.
const COMMANDER_WAR_COMMAND_COST = 20;
const COMMANDER_WAR_DAMAGE_BONUS = 0.30;
function applyCommanderWarBonus(dmg){
  if(state.playerClass !== 'commander' || state.commandPoints < COMMANDER_WAR_COMMAND_COST) return dmg;
  state.commandPoints -= COMMANDER_WAR_COMMAND_COST;
  return Math.round(dmg * (1 + COMMANDER_WAR_DAMAGE_BONUS));
}
...
async function contributeToWar(tid, side){
  ...
  const stats = getPlayerCombatStats();
  const roll = 0.85 + Math.random() * 0.3;
  const dmg = applyCommanderWarBonus(Math.max(1, Math.round(stats.atk * roll)));
...
function renderCoreRouteChain(kingdomId){
  ...
  const items = status.chain.map(node => {
    ...
    return { label: node.label, capital: node.capital, gate: !node.capital && node.idx === zoneCoreGateIndex(node.zone), state: nodeState, kd };
  });
...
/* ===== SEASON SYSTEM PANEL =====
   Sits right under the Core hero panel. meta/season (points per kingdom,
   startedAt/endsAt) is written server-side only — see functions/index.js
   awardSeasonPoints()/resolveDueSeason(). Points come from: holding the
   Core through a 30-min dividend tick, capturing an outpost, and winning
   a full Core siege (by far the biggest chunk). */
function renderSeasonPanel(){
  if(!db) return '';
  if(!seasonLoaded){ loadSeason(); return `<div class="panel" style="padding:16px;text-align:center;color:var(--dim);font-size:12px;">Loading season…</div>`; }
  const season = seasonData || { seasonNumber: 1, points: {}, history: [] };
  const endsAtMs = warTimestampMs(season.endsAt);
  const remainingMs = endsAtMs ? Math.max(0, endsAtMs - Date.now()) : 0;
  const ranked = Object.entries(season.points || {}).sort((a, b) => b[1] - a[1]);
  const hasHistory = (season.history || []).length > 0;

  const rows = ranked.length ? ranked.map(([kd, pts], i) => {
    const def = kingdomDef(kd);
    return `<div class="season-row" style="display:flex;align-items:center;gap:8px;padding:6px 4px;${i===0?'background:rgba(212,162,76,0.08);border-radius:6px;':''}">
      <span style="font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--dim);width:16px;">#${i+1}</span>
      <span style="width:22px;height:22px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:${def?def.color:'#555'};font-size:12px;flex-shrink:0;">${def?def.emblem:'❔'}</span>
      <span style="flex:1;font-size:12px;">${def?def.name:kd}</span>
      <span style="font-family:'JetBrains Mono',monospace;font-size:12px;font-weight:700;color:${i===0?'var(--brass-bright)':'var(--text)'};">${fmtG(Math.round(pts))}</span>
    </div>`;
  }).join('') : `<div style="padding:10px 4px;text-align:center;color:var(--dim);font-size:12px;">No Season Points scored yet — hold the Core, win a war, or win a Core siege.</div>`;

  return `<div class="panel" style="padding:14px 14px 10px;margin-top:10px;">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
      <div style="font-family:'Cairo',sans-serif;font-weight:800;font-size:14px;letter-spacing:0.04em;"><img class="ui-icon" src="${ICONS.calendar||ICONS.scroll_plain}" alt="📅"> SEASON ${season.seasonNumber || 1}</div>
      <div style="font-family:'JetBrains Mono',monospace;font-size:11px;color:${remainingMs < 86400000 ? 'var(--red)' : 'var(--dim)'};">${remainingMs > 0 ? formatWarCountdown(remainingMs) + ' left' : 'Ending soon…'}</div>
    </div>
    <div>${rows}</div>
    ${hasHistory ? `<button class="mini-btn" style="display:block;width:100%;margin-top:8px;text-align:center;" onclick="openSeasonHistoryModal()"><img class="ui-icon" src="${ICONS.scroll_plain}" alt="📜"> Season History</button>` : ''}
  </div>`;
}

function openSeasonHistoryModal(){ if(!seasonData || !(seasonData.history||[]).length) return; activeSeasonHistoryOpen = true; renderBody(); }
function closeSeasonHistoryModal(){ activeSeasonHistoryOpen = false; renderBody(); }

function renderSeasonHistoryModal(){
  if(!activeSeasonHistoryOpen || !seasonData) return '';
  const history = (seasonData.history || []).slice().reverse();
  const rows = history.map(h => {
    const def = kingdomDef(h.winnerKingdom);
    const endedMs = warTimestampMs(h.endedAt);
    return `<div style="display:flex;align-items:center;gap:10px;padding:10px 4px;border-bottom:1px solid var(--border);">
      <span style="width:30px;height:30px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:${def?def.color:'#555'};font-size:16px;flex-shrink:0;">${def?def.emblem:'❔'}</span>
      <div style="flex:1;">
        <div style="font-size:13px;font-weight:700;">Season ${h.seasonNumber} — ${def?def.name:'No winner'}</div>
        <div style="font-size:11px;color:var(--dim);">${fmtG(Math.round(h.winnerPoints||0))} Season Points${endedMs?' · '+new Date(endedMs).toLocaleDateString():''}</div>
      </div>
    </div>`;
  }).join('');
  return `<div class="war-modal-backdrop" onclick="if(event.target===this)closeSeasonHistoryModal()">
    <div class="war-modal" style="max-width:360px;">
      <button class="war-modal-close" onclick="closeSeasonHistoryModal()"><img class="ui-icon" src="${ICONS.close}" alt="✕"></button>
      <div class="war-modal-title"><img class="ui-icon" src="${ICONS.scroll_plain}" alt="📜"> Season History</div>
      <div>${rows || '<div style="padding:16px;text-align:center;color:var(--dim);">No completed seasons yet.</div>'}</div>
    </div>
  </div>`;
}
...
function bindWarTicker(){
  ...
  setInterval(() => {
    ...
    // Only worth the Firestore reads (+ possible renderBodySilently DOM
    // churn) while the player can actually see the result. Previously ran
    // unconditionally every 30s even mid-battle or while typing in the
    // market search box.
    if(activeTab === 'zones' || activeTab === 'battles'){
      pollTerritories();
      pollCore();
    }
  }, 30000);
...
/* ═══════════════════════════════════════════════════════════
   ...
   Shape (label/color/names) is derived from WORLD_CONFIG at the top
   of this file — kept as its own name because alliance.js and
   ui-render.js already read ARCADIA_WORLD[zone].{label,color,names}
   directly, so this stays the public shape those files expect.
   ═══════════════════════════════════════════════════════════ */
const ARCADIA_WORLD = {};
Object.keys(WORLD_CONFIG).forEach(z => {
  ARCADIA_WORLD[z] = { label: WORLD_CONFIG[z].label, color: WORLD_CONFIG[z].color, names: WORLD_CONFIG[z].regionNames };
});
...
function renderArcadiaMapSelection(){
  ...
  const rawIdx=parseInt(t.id.split('_').pop(),10), idx=Math.max(0,rawIdx-1), z=ARCADIA_WORLD[t.zone]||ARCADIA_WORLD.plains;
  ...
  const specialty = territorySpecialty(t.zone, rawIdx);
  const specialtyName = specialty && (typeof RESOURCES !== 'undefined' && RESOURCES[specialty]) ? RESOURCES[specialty].name : specialty;
  const effectiveTaxPct = Math.round((ZONE_TAX_RATE[t.zone]||0) * (1 + (mine && specialty ? TERRITORY_SPECIALTY_TAX_BONUS : 0)) * 100);
  panel.innerHTML=`<div class="map-info-title">${capital?`<img class="ui-icon" src="${ICONS.alliance}" alt="🏛"> `:''}${z.names[idx]}</div>
    ...
    ${specialty?`<div class="map-stat"><span>Deposit</span><b>${specialtyName} <span style="color:var(--brass-bright);">(+${Math.round(TERRITORY_SPECIALTY_TAX_BONUS*100)}% tax on ${specialtyName} zone-wide)</span></b></div>`:''}
    <div class="map-stat"><span>Tax</span><b>${effectiveTaxPct}%${specialty&&mine?' (deposit bonus applied)':''}</b></div>
...
function renderKingdomMapSVG(myKingdom){
  ...
  const svgTerritories=territories.map(o=>{
    ...
    return `<g class="world-territory ${selected?'selected':''}" data-territory="${o.meta.id}">
      ...
      ${o.idx===(zoneCoreGateIndex(o.zone)-1)?`<image class="capital-crown" href="${ICONS.castle}" x="${cx-8}" y="${cy-34}" width="16" height="16"/>`:''}
...
function renderActiveWarsList(){
  ...
  setTimeout(()=>{bindArcadiaMapInteractions(); renderArcadiaMapTransform(); renderArcadiaMapSelection();},0);
  return `<div class="wrap animate-fade">${renderCoreHeader()}${renderSeasonPanel()}${mapHtml}${typeof renderKingdomLeaderboardPanel==='function'?renderKingdomLeaderboardPanel():''}${tabsHtml}${body}</div>`;
}
