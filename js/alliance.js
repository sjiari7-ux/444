...
/* ===== PERMISSIONS ===== */
// V1: Kingdom management (promote/kick/demote/edit/inactivity + treasury +
// elections) is deferred to a later phase — see V1_CHANGES.md. Flipping this
// one flag back to true restores every permission and UI tab below exactly
// as it was; nothing else needs to change.
const KINGDOM_MANAGEMENT_ENABLED = false;
function allianceCan(role, perm){
  if(!KINGDOM_MANAGEMENT_ENABLED) return false;
  const perms = {
...
function renderAllianceDashboard(){
  ...
  // V1: Treasury/Manage tabs hidden — see KINGDOM_MANAGEMENT_ENABLED above.
  const tabs = [
    { id:'home',     label:`<img class="ui-icon" src="${ICONS.castle}" alt="🏰"> Home` },
    { id:'members',  label:`<img class="ui-icon" src="${ICONS.group}" alt="👥"> Members` },
  ];
  if(KINGDOM_MANAGEMENT_ENABLED) tabs.push({ id:'treasury', label:`<img class="ui-icon" src="${ICONS.business}" alt="💰"> Treasury` });
  ...
  else if(allianceView === 'treasury' && KINGDOM_MANAGEMENT_ENABLED) body = renderAllianceTreasury();
  else if(allianceView === 'manage' && KINGDOM_MANAGEMENT_ENABLED) body = renderAllianceManage();
    ...
    <div class="country-hero">
      <div class="country-hero-banner">
        <div class="country-hero-icons">
          ...
          <!-- V1: donate button hidden with Treasury (see KINGDOM_MANAGEMENT_ENABLED) -->
          ${KINGDOM_MANAGEMENT_ENABLED ? `<button class="alliance-icon-btn donate" title="Donate Gold" onclick="openAllianceDonateModal()"><img class="ui-icon" src="${ICONS.gold_coin}" alt="🪙"></button>` : ''}
        </div>
...
function renderAllianceOverview(){
  ...
  return `
    ...
    ${KINGDOM_MANAGEMENT_ENABLED ? renderElectionPanel() : ''}
