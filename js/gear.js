...
function generateGear(slot, tierIdx){
  ...
  const stats = {};
  const pool = (GEAR_SLOTS[slot] && GEAR_SLOTS[slot].allowedStats) || STAT_POOL;
  const chosen = [...pool].sort(()=>Math.random()-0.5).slice(0, Math.min(tier.numStats, pool.length));
  const [rMin, rMax] = tier.statRange;
  chosen.forEach(stat=>{
    const baseVal = SKILLS[stat].perLevel * (rMin + Math.random()*(rMax-rMin));
    stats[stat] = Math.max(1, Math.round(baseVal * tier.power));
  });
  const setId = GEAR_SLOTS[slot] && GEAR_SLOTS[slot].setEligible === false ? null : 'tier'+tierIdx;
  const gear = { id: Date.now()+Math.random(), slot, tier: tierIdx, upgradeLevel: 0, name, stats, setId };
...
function getSetBonusMultiplier(s, stat){
  const counts = getSetCounts(s);
  let mult = 1;
  Object.keys(counts).forEach(setId=>{
    const set = SET_DATA[setId];
    if(!set) return;
    const pieces = counts[setId];
    set.bonuses.forEach(b=>{
      if(pieces >= b.pieces && (b.stat === stat || b.stat === 'all')) mult += b.bonus;
    });
  });
  return mult;
}
...
function getGearBonus(s, stat){
  ...
  return total * getSetBonusMultiplier(s, stat);
}
...
// Forge recipe (e.g. CRAFTABLE_GEAR.armor tier1 "Iron Armor" <img class="ui-icon" src="${ICONS.arrow_h}" alt="↔">
...
function renderGear(){
  ...
  const gearMarketButton = `
    <div class="forge-cta">
      ...
      <div class="forge-desc">Buy and sell real crafted gear with other players — the item, its exact stats and upgrade level, hand to hand. Now part of the Player Market.</div>
      <button class="forge-open-btn" onclick="goToGearMarket()">Open Gear Market</button>
    </div>`;
  ...
  if(forgeOpen && forgeSlot === null){
    ...
    const tierRows = Object.keys(CRAFTABLE_GEAR).map(slot=>{
      ...
      const statusLabel = locked ? `<img class="ui-icon" src="${ICONS.lock}" alt="🔒"> Lv.${r.levelReq}` : (hasInputs ? 'craft ok' : 'missing');
      ...
      <div class="modal-overlay" onclick="if(event.target===this)closeForge()">
        <div class="modal-box forge-modal">
          <div class="modal-header">
            <h3><img class="ui-icon" src="${ICONS.craft}" alt="⚒"> The Forge</h3>
            <button class="modal-close" onclick="closeForge()"><img class="ui-icon" src="${ICONS.close}" alt="✕"></button>
          </div>
  ...
  if(forgeOpen && forgeSlot !== null){
    ...
    forgeModal = `
      <div class="modal-overlay" onclick="if(event.target===this)closeForge()">
        <div class="modal-box forge-modal">
          <div class="modal-header">
            <h3><img class="ui-icon" src="${ICONS.craft}" alt="⚒"> Forge — ${slotInfo.icon} ${slotInfo.name}</h3>
            <div style="display:flex;gap:8px;align-items:center;">
              <button class="mini-btn" onclick="backToForgeGrid()"><img class="ui-icon" src="${ICONS.arrow_left}" alt="←"> All Gear</button>
              <button class="modal-close" onclick="closeForge()"><img class="ui-icon" src="${ICONS.close}" alt="✕"></button>
            </div>
          </div>
          <div class="modal-body">
            <div class="forge-recipe-card ${stateClass}" style="--tc:${tier.color};">
              ...
              ${locked ? `<div class="locked-tag" style="text-align:center;padding:10px;"><img class="ui-icon" src="${ICONS.lock}" alt="🔒"> Requires player level ${recipe.levelReq}</div>` :
                `<button class="act-btn ${canCraft?'buy':''}" style="width:100%;" ${!canCraft?'disabled':''} onclick="craftGear('${forgeSlot}',${forgeTier})">
                  ${bagFull ? `<img class="ui-icon" src="${ICONS.bag_full}" alt="🎒"> Bag Full` : (!hasInputs ? `<img class="ui-icon" src="${ICONS.cancel}" alt="❌"> Missing Materials` : (!hasEnergy ? `<img class="ui-icon" src="${ICONS.energy}" alt="⚡"> Not Enough Energy` : `<img class="ui-icon" src="${ICONS.hammer}" alt="🔨"> Forge ${recipe.name}`))}
                </button>`}
            </div>
  ...
  const bagTiles = state.gearBag.map(item=>{
    ...
    return `<div class="gear-inv-tile ${bagSelected===item.id?'selected':''}" style="--tc:${t.color};" onclick="selectBagItem(${item.id})">
      <button class="tile-quick-sell" onclick="event.stopPropagation();openGearSellModal(${item.id})" title="List on Market"><img class="ui-icon" src="${ICONS.tag}" alt="🏷"></button>
  ...
  return `
    ${forgeModal}
    <div class="section-title"><h2><img class="ui-icon" src="${ICONS.defense_ui}" alt="🛡"> Equipped Gear</h2></div>
    <div class="paperdoll">
    ...
    <div class="section-title"><h2><img class="ui-icon" src="${ICONS.craft}" alt="⚒"> Forge</h2><div class="sub">Craft gear from materials</div></div>
...
function setForgeSearch(val){
  ...
  // Same fix as setMarketSearch (js/main.js): a full renderBody() per
  // keystroke was rebuilding the WHOLE app for every character typed while
  // filtering a gear bag that keeps growing (storageCap scales with level),
  // which is the actual felt lag. Debounce the rebuild, not the keystroke.
  clearTimeout(_forgeSearchTimer);
  _forgeSearchTimer = setTimeout(() => {
    renderBody();
...
function craftGear(slot, tier){
  ...
  // Generate stats based on tier — bounded to this slot's allowedStats and the tier's statRange
  const t = GEAR_TIERS[tier];
  const pool = (GEAR_SLOTS[slot] && GEAR_SLOTS[slot].allowedStats) || STAT_POOL;
  const chosen = [...pool].sort(()=>Math.random()-0.5).slice(0, Math.min(t.numStats, pool.length));
  const [rMin, rMax] = t.statRange;
  chosen.forEach(st=>{
    const baseVal = SKILLS[st].perLevel * (rMin + Math.random()*(rMax-rMin));
    gear.stats[st] = Math.max(1, Math.round(baseVal * t.power));
  });
  gear.setId = GEAR_SLOTS[slot] && GEAR_SLOTS[slot].setEligible === false ? null : 'tier'+tier;
...
/* ═══════════════════════════════════════════════════════════════
   ...
   The browse/mine rows below are now rendered inline inside the
   Player Market panel (see renderGearMarketSection in ui-render.js)
   instead of their own modal, so Resources and Gear live side by
   side in one place.
   ═══════════════════════════════════════════════════════════════ */

// Jumps from the Gear tab straight into the Gear side of the Player
// Market, loading fresh listings for whatever sub-view was last open.
function goToGearMarket(){
  activeTab = 'market';
  state.marketCategory = 'gear';
  renderBody();
...
function renderMyGearListingsRows(){
  ...
  return state.myGearListingsCache.map(l=>{
    ...
    return `<div class="market-row">
      ...
      <div class="market-actions">
        <button class="mini-btn sell" onclick="cancelGearListing('${l.id}')"><img class="ui-icon" src="${ICONS.close}" alt="✕"> Cancel & return</button>
      </div>
    </div>`;
  }).join('');
}

function renderMyGearBagRows(){
  if(!state.gearBag.length){
    return `<div style="padding:30px 20px;text-align:center;color:var(--dim);font-size:13px;">Your bag is empty. Forge something, or defeat monsters to find gear — then it'll show up here ready to list.</div>`;
  }
  return state.gearBag.map(item=>{
    const t = GEAR_TIERS[item.tier];
    const info = GEAR_SLOTS[item.slot];
    const eff = getGearEffectiveStats(item);
    const statsLine = Object.keys(eff).map(st=>{
    ...
    return `<div class="market-row">
      <div class="market-left">
        <div class="card-icon" style="font-size:26px;">${info.icon}</div>
        <div>
          <div class="card-name" style="color:${t.color};">[${t.symbol}] ${escapeHtml(item.name)} +${item.upgradeLevel}</div>
          <div class="market-owned">${statsLine || 'No bonus stats'}</div>
        </div>
      </div>
      <div class="market-actions">
        <button class="mini-btn buy" onclick="openGearSellModal(${item.id})"><img class="ui-icon" src="${ICONS.tag}" alt="🏷"> Set price & list</button>
      </div>
    </div>`;
  }).join('');
}
...
function renderGearSellModal(gearId){
  ...
  const instantSell = getGearValue(gear);
  return `
    <div class="modal-overlay" onclick="if(event.target===this)closeGearSellModal()">
      <div class="modal-box">
        <div class="modal-header"><h3>${info.icon} List ${escapeHtml(gear.name)}</h3><button class="modal-close" onclick="closeGearSellModal()"><img class="ui-icon" src="${ICONS.close}" alt="✕"></button></div>
        <div class="modal-body">
          <div class="market-owned" style="margin-bottom:12px;color:${t.color};">[${t.symbol}] +${gear.upgradeLevel} · Instant sell to the game would give ${instantSell}g</div>
          <label style="font-size:11px;color:var(--dim);display:block;margin-bottom:4px;">Price (g) — set your own, no suggested value</label>
          <input type="number" id="gearSellPriceInput" class="username-input" min="1" step="1" placeholder="Enter a price...">
          <div id="gearSellPriceError" style="display:none;color:var(--danger,#e05252);font-size:11.5px;margin-top:6px;"><img class="ui-icon" src="${ICONS.warning}" alt="⚠"> Enter a price above 0 first.</div>
...
function submitGearSellForm(gearId){
  ...
  const errEl = document.getElementById('gearSellPriceError');
  const price = priceEl ? Number(priceEl.value) : 0;
  if(!(price > 0)){
    // Catch this here, not just inside listGearForSale — that path only
    // logs to the internal activity log without re-rendering, so an empty
    // or zero price used to fail with zero visible feedback: the modal
    // just sat there and the item silently never got listed.
