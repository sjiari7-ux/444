...
function renderGear(){
  ...
  const bagTiles = state.gearBag.map(item=>{
    ...
    return `<div class="gear-inv-tile ${bagSelected===item.id?'selected':''}" style="--tc:${t.color};" onclick="selectBagItem(${item.id})">
      <button class="tile-quick-sell" onclick="event.stopPropagation();openGearSellModal(${item.id})" title="List on Market">🏷️</button>
...
function renderMyGearBagRows(){
  ...
  return state.gearBag.map(item=>{
    ...
    return `<div class="market-row">
      ...
      <div class="market-actions">
        <button class="mini-btn buy" onclick="openGearSellModal(${item.id})">🏷️ Set price & list</button>
      </div>
    </div>`;
  }).join('');
}
    ...
    <div class="modal-overlay" onclick="if(event.target===this)closeGearSellModal()">
      <div class="modal-box">
        ...
        <div class="modal-body">
          ...
          <div id="gearSellPriceError" style="display:none;color:var(--danger,#e05252);font-size:11.5px;margin-top:6px;">⚠️ Enter a price above 0 first.</div>
