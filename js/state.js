...
const ZONE_MONSTERS = {
    plains: [
        { name:'Wild Rabbit', nameAr:'Wild Rabbit', icon:`<img class="ui-icon" src="${ICONS.rabbit}" alt="🐰">`, level:1,  hp:20,  atkMin:3,  atkMax:5,  def:1,  spd:25, crit:0.02, dodge:0.15, xp:5,   goldMin:2,  goldMax:4,  loot:[{item:'food',min:1,max:2}] },
        { name:'Wolf',        nameAr:'Wolf',       icon:`<img class="ui-icon" src="${ICONS.wolf}" alt="🐺">`, level:5,  hp:40,  atkMin:8,  atkMax:12, def:3,  spd:35, crit:0.05, dodge:0.10, xp:15,  goldMin:5,  goldMax:10, loot:[{item:'food',min:1,max:3},{item:'leather',min:0,max:1}] },
    ...
    forest: [
        { name:'Snake',       nameAr:'Snake',     icon:`<img class="ui-icon" src="${ICONS.snake}" alt="🐍">`, level:12, hp:80,  atkMin:15, atkMax:20, def:8,  spd:40, crit:0.08, dodge:0.20, xp:35,  goldMin:12, goldMax:20, loot:[{item:'food',min:0,max:2},{item:'leather',min:1,max:2}] },
        { name:'Bear',        nameAr:'Bear',        icon:`<img class="ui-icon" src="${ICONS.bear}" alt="🐻">`, level:18, hp:150, atkMin:20, atkMax:30, def:15, spd:25, crit:0.05, dodge:0.05, xp:60,  goldMin:25, goldMax:40, loot:[{item:'wood',min:3,max:6},{item:'leather',min:1,max:3}] },
        { name:'Imp',         nameAr:'Imp',     icon:`<img class="ui-icon" src="${ICONS.imp}" alt="👿">`, level:22, hp:120, atkMin:25, atkMax:35, def:10, spd:45, crit:0.12, dodge:0.15, xp:75,  goldMin:30, goldMax:50, loot:[{item:'wood',min:2,max:4},{item:'food',min:1,max:3}] },
    ],
    mountain: [
        { name:'Eagle',       nameAr:'Eagle',       icon:`<img class="ui-icon" src="${ICONS.eagle}" alt="🦅">`, level:28, hp:160, atkMin:30, atkMax:40, def:12, spd:55, crit:0.10, dodge:0.25, xp:90,  goldMin:35, goldMax:55, loot:[{item:'stone',min:2,max:5},{item:'coal',min:0,max:2}] },
        { name:'Wild Climber',nameAr:'Wild Climber',icon:`<img class="ui-icon" src="${ICONS.climber}" alt="🧗">`,level:32, hp:200, atkMin:35, atkMax:50, def:20, spd:30, crit:0.06, dodge:0.10, xp:120, goldMin:50, goldMax:80, loot:[{item:'iron',min:2,max:5},{item:'stone',min:3,max:6}] },
        { name:'Stone Giant', nameAr:'Stone Giant',icon:`<img class="ui-icon" src="${ICONS.moai}" alt="🗿">`,level:38, hp:350, atkMin:40, atkMax:60, def:35, spd:15, crit:0.04, dodge:0.03, xp:180, goldMin:70, goldMax:110,loot:[{item:'stone',min:5,max:10},{item:'coal',min:2,max:5},{item:'iron',min:0,max:3}] },
    ],
    cave: [
        { name:'Giant Bat',   nameAr:'Giant Bat',icon:`<img class="ui-icon" src="${ICONS.bat}" alt="🦇">`,level:42, hp:200, atkMin:40, atkMax:55, def:18, spd:50, crit:0.10, dodge:0.20, xp:140, goldMin:55, goldMax:90, loot:[{item:'gemstones',min:1,max:3},{item:'gold',min:0,max:2}] },
        { name:'Dark Ghoul',  nameAr:'Dark Ghoul', icon:`<img class="ui-icon" src="${ICONS.skull}" alt="💀">`, level:48, hp:300, atkMin:50, atkMax:70, def:25, spd:35, crit:0.08, dodge:0.10, xp:200, goldMin:80, goldMax:130,loot:[{item:'gemstones',min:2,max:5},{item:'gold',min:1,max:3}] },
        { name:'Young Dragon',nameAr:'Young Dragon',icon:`<img class="ui-icon" src="${ICONS.dragon}" alt="🐉">`,level:52, hp:450, atkMin:60, atkMax:90, def:30, spd:40, crit:0.15, dodge:0.12, xp:280, goldMin:120,goldMax:180,loot:[{item:'gold',min:3,max:6},{item:'gemstones',min:2,max:4},{item:'magic_stones',min:0,max:2}] },
    ],
    swamp: [
        { name:'Swamp Dragon',nameAr:'Swamp Dragon',icon:`<img class="ui-icon" src="${ICONS.dragon_face}" alt="🐲">`,level:58, hp:500, atkMin:70, atkMax:100,def:35, spd:35, crit:0.12, dodge:0.10, xp:350, goldMin:150,goldMax:220,loot:[{item:'magic_stones',min:1,max:3},{item:'gemstones',min:2,max:5}] },
    ...
    dark: [
        { name:'Demon',       nameAr:'Demon',     icon:`<img class="ui-icon" src="${ICONS.devil}" alt="😈">`, level:72, hp:600, atkMin:90, atkMax:120,def:40, spd:45, crit:0.15, dodge:0.15, xp:500, goldMin:220,goldMax:320,loot:[{item:'magic_stones',min:2,max:5},{item:'gemstones',min:3,max:6},{item:'gold',min:4,max:8}] },
        { name:'Ancient Dragon',nameAr:'Ancient Dragon',icon:`<img class="ui-icon" src="${ICONS.dragon}" alt="🐉">`,level:78, hp:800, atkMin:100,atkMax:140,def:50, spd:40, crit:0.20, dodge:0.12, xp:650, goldMin:300,goldMax:420,loot:[{item:'magic_stones',min:3,max:7},{item:'gemstones',min:4,max:8},{item:'gold',min:5,max:10}] },
        { name:'Lich',        nameAr:'Lich',       icon:`<img class="ui-icon" src="${ICONS.skull}" alt="💀">`, level:85, hp:700, atkMin:110,atkMax:150,def:35, spd:55, crit:0.25, dodge:0.18, xp:750, goldMin:350,goldMax:500,loot:[{item:'magic_stones',min:4,max:8},{item:'gemstones',min:5,max:10},{item:'gold',min:6,max:12}] },
    ],
};
