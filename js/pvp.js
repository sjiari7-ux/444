/* ═══════════════════════════════════════════════════════════════
   ...
   <img class="ui-icon" src="${ICONS.warning}" alt="⚠"> FIRESTORE RULE NEEDED — add this so step 3 works:
     match /players/{uid} {
...
async function openPvpBattle(uid){
  ...
  try{
    ...
    state.classResource = 0; // battle-only meter starts empty every duel
    state.mageExhaustedTurns = 0;
    ...
    updateMissionProgress('pvp_attacks', 1);
  }catch(e){
...
function pvpPlayerAttackAction(isSkill, skillKey){
  ...
  if(isSkill && skillKey){
    ...
    } else if(skillKey === 'warBanner'){
      state.mana -= 12; dmg = Math.max(1, Math.round(pStats.atk * 2.2)); isPierce = true;
  ...
  if(state.playerClass === 'mage' && state.mageExhaustedTurns > 0){ dmg = Math.round(dmg * 0.7); }
  o.hp -= dmg;
  gainClassResourceOnHit(state, isCrit);
...
/* ===== PVP CLASS ULTIMATES (mirrors combat.js exactly, aimed at the opponent) ===== */
function pvpUseClassUltimate(){
  if(!pvpBattleState || pvpBattleState.won || pvpBattleState.lost || pvpBattleState.fled) return;
  if(!state.playerClass) return;
  const meta = getResourceMeta(state);
  if(!meta || !meta.ultimate) return;
  if(!isUltimateReady(state)){ pushLog(state, `Not enough ${meta.nameAr} yet!`, 'lose'); return; }
  const bs = pvpBattleState; const pStats = bs.playerStats; const o = bs.opponent;
  let dmg = 1;
  switch(meta.ultimate.key){
    case 'berserkerStrike':
      dmg = Math.max(1, Math.round(pStats.atk * 4 * (1 - (o.def * 0.3) / Math.max(1, pStats.atk))));
      state.classResource = 0;
      break;
    case 'perfectShot':
      dmg = Math.max(1, Math.round(pStats.atk * 2.6));
      state.classResource = 0;
      break;
    case 'rally':
      dmg = Math.max(1, Math.round((pStats.atk - o.def * (1 - (pStats.pierce||0))) * 1.8));
      state.commandPoints = Math.max(0, state.commandPoints - 40);
      break;
    case 'luckyDraw':
      dmg = Math.max(1, Math.round(pStats.atk - o.def * (1 - (pStats.pierce||0))));
      bs.rewards = bs.rewards || {};
      bs.rewards.bonusGold = Math.round((bs.defenderSnap.gold || 0) * 0.05);
      state.fortune = Math.max(0, state.fortune - 75);
      break;
  }
  o.hp -= dmg;
  bs.turns.push({ who:'player', dmg, crit:true, pierce:true, ultimate:true, skill: meta.ultimate.nameAr });
  if(o.hp <= 0){ o.hp = 0; bs.won = true; pvpAwardBattleRewards(); renderBody(); pvpPlayBattleEffects(bs); return; }
  bs.turnCount++; pvpAfterPlayerAction();
}

function pvpCastOverloadBlast(){
  if(!pvpBattleState || pvpBattleState.won || pvpBattleState.lost || pvpBattleState.fled) return;
  if(state.playerClass !== 'mage') return;
  const bs = pvpBattleState; const pStats = bs.playerStats; const o = bs.opponent;
  const cost = 30;
  if(state.mana < cost){ pushLog(state, 'Not enough mana to Overload!', 'lose'); return; }
  state.mana -= cost;
  const dmg = Math.max(1, Math.round(pStats.atk * 4.2));
  o.hp -= dmg;
  o.burnTurns = 4; o.burnDmg = Math.round(pStats.atk * 0.4);
  if(state.mana <= 0){ state.mageExhaustedTurns = 3; pushLog(state, 'Mana drained dry — Exhausted!', 'lose'); }
  bs.turns.push({ who:'player', dmg, crit:true, pierce:true, skill: 'Overload Blast' });
  if(o.hp <= 0){ o.hp = 0; bs.won = true; pvpAwardBattleRewards(); renderBody(); pvpPlayBattleEffects(bs); return; }
  bs.turnCount++; pvpAfterPlayerAction();
}
...
function pvpUseSkill(skillKey){
  ...
  else if(skillKey === 'warBanner') manaCost = 12;
...
function pvpOpponentAttackAction(){
  ...
  gainClassResourceOnHurt(state, isCrit);
  bs.turns.push({ who:'opponent', dmg, crit:isCrit });
  if(bs.playerHP <= 0){ bs.playerHP = 0; state.health = 0; bs.lost = true; updateClassResourceOnBattleEnd(state, false); pvpApplyLoss(); }
...
function pvpAwardBattleRewards(){
  ...
  let goldChange = Math.round(d.gold * stealPct) + ((bs.rewards && bs.rewards.stolenGold) || 0) + ((bs.rewards && bs.rewards.bonusGold) || 0);
  ...
  state.combat.wins += 1;
  state.pvp.wins += 1;
  gainFortuneFromLootValue(state, goldChange);
  updateClassResourceOnBattleEnd(state, true);
...
function renderPvpBattle(){
  ...
  const logLines = bs.turns.slice(-4).map(t => {
    ...
    else if(t.who==='player' && t.type==='defend'){ txt=`<img class="ui-icon" src="${ICONS.defense_ui}" alt="🛡"> Defending`; color='var(--skill)'; }
    else if(t.who==='player' && t.type==='flee' && t.success){ txt=`<img class="ui-icon" src="${ICONS.flee}" alt="🏃"> Retreated!`; color='var(--green)'; }
    else if(t.who==='player' && t.type==='flee' && !t.success){ txt=`<img class="ui-icon" src="${ICONS.flee}" alt="🏃"> Retreat failed`; color='var(--red)'; }
    else if(t.who==='skill' && t.type==='steal'){ txt=`<img class="ui-icon" src="${ICONS.business}" alt="💰"> +${t.gold}g`; color='var(--brass-bright)'; }
    else if(t.who==='skill' && t.type==='heal'){ txt=`<img class="ui-icon" src="${ICONS.heal}" alt="💚"> +${t.heal} HP`; color='var(--health)'; }
    else if(t.who==='player' && t.skill){ txt=`<img class="ui-icon" src="${ICONS.sparkle}" alt="✨"> ${t.skill} ${t.dmg} dmg`; color='var(--prestige)'; }
    else if(t.who==='player' && t.dmg>0){ txt=`<img class="ui-icon" src="${ICONS.damage_ui}" alt="⚔"> ${t.dmg}${t.crit?' <img class="ui-icon" src="${ICONS.impact}" alt="💥">':''}${t.charge?' ('+t.charge.toFixed(1)+'×)':''}`; color='var(--green)'; }
    else if(t.who==='opponent' && t.dmg>0){ txt=`<img class="ui-icon" src="${ICONS.blood_drop}" alt="🩸"> ${t.dmg}${t.crit?' <img class="ui-icon" src="${ICONS.impact}" alt="💥">':''}`; color='var(--red)'; }
    else if(t.who==='opponent' && t.dodge){ txt=`<img class="ui-icon" src="${ICONS.dash}" alt="💨"> Dodged!`; color='var(--skill)'; }
    else if(t.who==='burn'){ txt=`<img class="ui-icon" src="${ICONS.fire_streak}" alt="🔥"> Burn ${t.dmg}`; color='var(--copper)'; }
    else if(t.who==='item' && t.type==='health'){ txt=`<img class="ui-icon" src="${ICONS.potion}" alt="🧪"> +${t.heal} HP`; color='var(--health)'; }
  ...
  const resultHtml = bs.won ? `
    ...
    : bs.fled ? `
    <div style="position:absolute;inset:0;background:rgba(5,12,10,0.88);backdrop-filter:blur(4px);z-index:10;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:14px;padding:20px;text-align:center;animation:fadeIn 0.3s ease-out;">
      <div style="font-size:56px;"><img class="ui-icon" src="${ICONS.flee}" alt="🏃"></div>
  ...
  const skillBtn = cls ? (()=>{
    ...
    else if(sk.key==='warBanner') manaCost = 12;
  ...
  const itemMenuHtml = pvpItemMenuOpen ? `
    <div style="position:absolute;bottom:0;left:0;right:0;background:var(--panel);border-top:1px solid var(--border);border-radius:16px 16px 0 0;padding:16px;z-index:20;animation:slideUp 0.25s ease-out;" onclick="event.stopPropagation()">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
        <div style="font-size:13px;color:var(--dim);font-family:'Cairo',sans-serif;font-weight:700;"><img class="ui-icon" src="${ICONS.potion}" alt="🧪"> Use Item</div>
        <button style="background:none;border:none;color:var(--dim);font-size:18px;cursor:pointer;" onclick="pvpToggleItems()"><img class="ui-icon" src="${ICONS.close}" alt="✕"></button>
      </div>
      <div style="display:flex;flex-wrap:wrap;gap:6px;">
        ...
        ${(state.inv.health_potion||0)>0 ? `<button class="mini-btn" style="border-color:var(--health);color:var(--health);padding:8px 12px;" onclick="pvpUseBattleItem('health_potion')"><img class="ui-icon" src="${ICONS.heal}" alt="💚"> Health Potion +30-50HP ×${state.inv.health_potion}</button>` : ''}
      </div>
    </div>
  ` : '';
    ...
    <div style="position:absolute;top:0;left:0;right:0;bottom:0;background:rgba(5,12,10,0.92);backdrop-filter:blur(6px);z-index:20;display:flex;align-items:center;justify-content:center;padding:20px;animation:fadeIn 0.2s ease-out;" onclick="if(event.target===this)pvpToggleCodex()">
      <div style="background:var(--panel);border:1px solid var(--border);border-radius:12px;padding:20px;width:100%;max-width:360px;" onclick="event.stopPropagation()">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">
          ...
          <button style="background:none;border:none;color:var(--dim);font-size:18px;cursor:pointer;" onclick="pvpToggleCodex()"><img class="ui-icon" src="${ICONS.close}" alt="✕"></button>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:12px;color:var(--dim);margin-bottom:12px;">
          ...
          <div><img class="ui-icon" src="${ICONS.damage_ui}" alt="⚔"> ATK: <b style="color:var(--text);">${o.atk}</b></div>
          <div><img class="ui-icon" src="${ICONS.defense_ui}" alt="🛡"> DEF: <b style="color:var(--text);">${(o.def*100).toFixed(0)}%</b></div>
          <div><img class="ui-icon" src="${ICONS.dash}" alt="💨"> SPD: <b style="color:var(--text);">${o.spd}</b></div>
          <div><img class="ui-icon" src="${ICONS.spellbook}" alt="🎯"> Crit: <b style="color:var(--text);">${(o.crit*100).toFixed(0)}%</b></div>
          <div><img class="ui-icon" src="${ICONS.dizzy}" alt="💫"> Dodge: <b style="color:var(--text);">${(o.dodge*100).toFixed(0)}%</b></div>
        </div>
      </div>
    </div>
  ` : '';
  ...
  return `<div style="position:relative;display:flex;flex-direction:column;height:100%;min-height:calc(100vh - 60px);background:linear-gradient(180deg,#080b10 0%,#131a24 40%,#1a2530 100%);overflow:hidden;">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;padding:12px 16px;z-index:5;">
      ...
      <div style="display:flex;align-items:center;gap:8px;">
        <div class="${canAct?'animate-pulse':''}" style="font-family:'Cairo',sans-serif;font-weight:800;font-size:11px;letter-spacing:0.06em;padding:5px 10px;border-radius:20px;background:${canAct?'rgba(255,195,92,0.15)':'rgba(196,76,76,0.15)'};color:${canAct?'var(--green)':'var(--red)'};border:1px solid ${canAct?'var(--green)':'var(--red)'};text-transform:uppercase;">${canAct?`<img class="ui-icon" src="${ICONS.energy}" alt="⚡"> Your Turn`:`<img class="ui-icon" src="${ICONS.damage_ui}" alt="⚔"> Rival Turn`}</div>
    ...
    <div style="flex:1;display:flex;flex-direction:column;justify-content:center;align-items:center;position:relative;padding:0 20px;gap:24px;">
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-end;width:100%;max-width:420px;position:relative;z-index:2;">
        ...
        <div style="display:flex;flex-direction:column;align-items:center;gap:6px;">
          ...
          <div style="font-size:10px;color:var(--dim);font-family:'JetBrains Mono',monospace;">${Math.floor(state.mana)} MP${state.playerClass==='mage' && state.mageExhaustedTurns>0 ? ` · <span style="color:var(--red);">Exhausted ${state.mageExhaustedTurns}t</span>` : ''}</div>
          ${(() => {
            const rMeta = getResourceMeta(state);
            if(!rMeta || rMeta.key === 'mana') return '';
            const cur = getClassResourceAmount(state);
            const max = getClassResourceMax(state);
            const pct = max ? Math.min(100, (cur/max)*100) : 0;
            const ready = isUltimateReady(state);
            return `
          <div style="width:90px;height:6px;background:#2a1f10;border-radius:3px;overflow:hidden;border:1px solid #4a3a20;margin-top:2px;">
            <div style="width:${pct}%;height:100%;background:${rMeta.color};transition:width 0.4s ease-out;${ready?'box-shadow:0 0 6px '+rMeta.color+';':''}"></div>
          </div>
          <div style="font-size:9px;color:${ready?rMeta.color:'var(--dim)'};font-family:'JetBrains Mono',monospace;font-weight:${ready?'700':'400'};">${Math.floor(cur)}${max?'/'+max:''} ${rMeta.nameAr}</div>`;
          })()}
        </div>
        ...
        <div style="display:flex;flex-direction:column;align-items:center;gap:4px;">
          ...
          ${bs.isDefending ? `<div style="font-size:11px;color:var(--skill);font-weight:700;"><img class="ui-icon" src="${ICONS.defense_ui}" alt="🛡"> Defending</div>` : ''}
          ${o.burnTurns > 0 ? `<div style="font-size:11px;color:var(--copper);font-weight:700;"><img class="ui-icon" src="${ICONS.fire_streak}" alt="🔥"> Burn ${o.burnTurns}t</div>` : ''}
        </div>
    ...
    <div style="padding:12px 16px 24px;display:flex;flex-direction:column;gap:8px;z-index:5;">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
        ...
        <button class="battle-btn" style="${!canAct?'opacity:0.35;':''}" ${!canAct?'disabled':''} onclick="pvpPlayerAttackAction(false)">
          <div style="font-size:22px;margin-bottom:2px;"><img class="ui-icon" src="${ICONS.damage_ui}" alt="⚔"></div>
        ...
        <button class="battle-btn" style="border-color:${skillBtn && !skillBtn.disabled ? 'var(--prestige)' : 'var(--border)'};color:${skillBtn && !skillBtn.disabled ? 'var(--prestige)' : 'var(--dim)'};${(skillBtn && skillBtn.disabled) || !canAct ? 'opacity:0.35;' : ''}" ${(skillBtn && skillBtn.disabled) || !canAct ? 'disabled' : ''} onclick="pvpUseSkill('${skillBtn ? skillBtn.key : ''}')">
          <div style="font-size:22px;margin-bottom:2px;"><img class="ui-icon" src="${ICONS.sparkle}" alt="✨"></div>
        ...
        <button class="battle-btn" style="${!canAct?'opacity:0.35;':''}" ${!canAct?'disabled':''} onclick="pvpAttemptFlee()">
          <div style="font-size:22px;margin-bottom:2px;"><img class="ui-icon" src="${ICONS.boots}" alt="👢"></div>
        ...
        <button class="battle-btn" style="${!canAct?'opacity:0.35;':''}" ${!canAct?'disabled':''} onclick="pvpToggleItems()">
          <div style="font-size:22px;margin-bottom:2px;"><img class="ui-icon" src="${ICONS.potion}" alt="🧪"></div>
      ...
      ${(() => {
        ...
        if(!rMeta) return '';
        if(rMeta.key === 'mana'){
          const canOverload = canAct && state.mana >= 30;
          return `
      <button class="battle-btn" style="margin-top:2px;border-color:${canOverload?rMeta.color:'var(--border)'};color:${canOverload?rMeta.color:'var(--dim)'};${!canOverload?'opacity:0.35;':''}" ${!canOverload?'disabled':''} onclick="pvpCastOverloadBlast()">
        <div style="font-family:'Cairo',sans-serif;font-weight:800;font-size:14px;letter-spacing:0.06em;">${rMeta.overload.icon} ${rMeta.overload.name.toUpperCase()} <span style="font-size:10px;opacity:0.7;">(30 MP)</span></div>
      </button>`;
        }
        if(!rMeta.ultimate) return '';
        const ready = canAct && isUltimateReady(state);
        return `
      <button class="battle-btn" style="margin-top:2px;border-color:${ready?rMeta.color:'var(--border)'};color:${ready?rMeta.color:'var(--dim)'};${!ready?'opacity:0.35;':''}${ready?'box-shadow:0 0 10px '+rMeta.color+';':''}" ${!ready?'disabled':''} onclick="pvpUseClassUltimate()">
        <div style="font-family:'Cairo',sans-serif;font-weight:800;font-size:14px;letter-spacing:0.06em;">${rMeta.ultimate.icon} ${rMeta.ultimate.name.toUpperCase()}</div>
      </button>`;
      })()}
      <!-- Forfeit row -->
      <div style="display:flex;justify-content:center;gap:8px;margin-top:4px;">
        <button class="mini-btn" style="font-size:10px;padding:4px 12px;opacity:0.6;" onclick="pvpForfeitBattle()"><img class="ui-icon" src="${ICONS.flee}" alt="🏃"> Forfeit</button>
      </div>
    </div>
...
function renderPvpTab(){
  if(state.level < PVP_MIN_LEVEL){
    return `<div class="wrap animate-fade"><div class="panel" style="padding:20px;text-align:center;">
      <div style="font-size:32px;margin-bottom:8px;"><img class="ui-icon" src="${ICONS.damage_ui}" alt="⚔"></div>
  ...
  const tierIdx = PVP_TIERS.indexOf(myTier);
  const nextTier = PVP_TIERS[tierIdx + 1] || null;
  const progressPct = nextTier ? Math.min(100, Math.max(0, ((myRating - myTier.min) / (nextTier.min - myTier.min)) * 100)) : 100;
  const ratingToNext = nextTier ? Math.max(0, nextTier.min - myRating) : 0;
  const header = `
    <div class="wrap animate-fade" style="padding-bottom:0;">
      ...
      <div class="panel" style="padding:12px;text-align:center;margin-bottom:8px;">
        ...
        <div class="bar-track" style="height:8px;margin-top:10px;"><div class="bar-fill" style="width:${progressPct}%;background:${nextTier ? nextTier.color : myTier.color};"></div></div>
        <div style="font-size:11px;color:var(--dim);margin-top:6px;">
          ${nextTier ? `${ratingToNext} rating to <b style="color:${nextTier.color};">${nextTier.name}</b>` : 'Max rank reached'}
        </div>
        <div style="display:flex;justify-content:center;gap:4px;margin-top:10px;flex-wrap:wrap;">
          ${PVP_TIERS.map((t, i) => `
            <div style="display:flex;flex-direction:column;align-items:center;gap:2px;opacity:${i <= tierIdx ? '1' : '0.4'};">
              <span style="width:8px;height:8px;border-radius:50%;background:${t.color};box-shadow:${i === tierIdx ? `0 0 0 2px var(--panel), 0 0 0 3px ${t.color}` : 'none'};"></span>
              <span style="font-size:8px;color:${i === tierIdx ? t.color : 'var(--dim)'};font-weight:${i === tierIdx ? '700' : '400'};">${t.name}</span>
              <span style="font-size:7px;color:var(--dim);font-family:'JetBrains Mono',monospace;">${t.min}</span>
            </div>`).join('')}
        </div>
        <div style="margin-top:10px;"><span style="cursor:pointer;font-size:11px;color:var(--brass-bright);" onclick="openPvpRankTop100()"><img class="ui-icon" src="${ICONS.medal_gold}" alt="🥇"> Top 100</span></div>
      </div>
  ...
  if(pvpOpponentsLoading && !pvpOpponents){
    ...
    body = `<div class="wrap"><div class="lb-note"><img class="ui-icon" src="${ICONS.warning}" alt="⚠"> ${pvpOpponentsError}</div><button class="btn btn-secondary" style="margin-top:8px;" onclick="refreshPvpOpponents()">Retry</button></div>`;
  } else if(!pvpOpponents || pvpOpponents.length === 0){
    body = `<div class="wrap"><div class="lb-note">No challengers found in your level range right now.</div><button class="btn btn-secondary" style="margin-top:8px;" onclick="refreshPvpOpponents()"><img class="ui-icon" src="${ICONS.reload}" alt="↻"> Refresh</button></div>`;
  } else {
    const range = pvpLevelRange();
...
/* ═══════════════════════════════════════════════════════════════
   Top 100 PvP Rank — full board, opened from the RANK panel above.
   Uses showModal() (appended to document.body, not #app) so it isn't
   torn down by unrelated renderBody() calls elsewhere in the game —
   same reasoning as patchMarketDetailModal()/patchWarDetailModal().
   ═══════════════════════════════════════════════════════════════ */
function renderPvpRankTop100Body(){
  if(pvpRankTop100Loading && !pvpRankTop100){
    return `<div class="lb-note">Loading Top 100…</div>`;
  }
  if(pvpRankTop100Error && !pvpRankTop100){
    return `<div class="lb-note"><img class="ui-icon" src="${ICONS.warning}" alt="⚠"> ${pvpRankTop100Error}</div>
      <button class="btn btn-secondary" style="margin-top:8px;" onclick="loadPvpRankTop100(true).then(patchPvpRankTop100Modal)">Retry</button>`;
  }
  const list = pvpRankTop100 || [];
  if(!list.length) return `<div class="lb-note">No ranked players yet.</div>`;
  return `
    <div class="lb-note" style="margin-bottom:8px;"><img class="ui-icon" src="${ICONS.globe}" alt="🌐"> Live, top ${list.length} by PvP rating.
      <span style="cursor:pointer;color:var(--brass-bright);" onclick="loadPvpRankTop100(true).then(patchPvpRankTop100Modal)"><img class="ui-icon" src="${ICONS.reload}" alt="↻"> Refresh</span>
    </div>
    <div class="rank-head"><span class="rh-num">#</span><span class="rh-avatar"></span><span class="rh-id">Player</span><span class="rh-value">Rating</span></div>
    <div class="rank-list" style="max-height:60vh;overflow-y:auto;">
      ${list.map((r,i)=>{
        const rank = i+1;
        const tierClass = rank===1?'top1':rank===2?'top2':rank===3?'top3':'';
        const tier = pvpTierFor(r.pvpRating);
        return `<div class="rank-row ${tierClass} ${r.me?'me':''}" onclick="viewChatProfile('${r.uid}')">
          ...
          <span class="rank-id">
            ...
            <span class="rank-sub" style="color:${tier.color};">${tier.name}</span>
          </span>
          <span class="rank-value">${r.pvpRating}</span>
        </div>`;
      }).join('')}
    </div>`;
}
// Updates just the modal-body in place (see patchMarketDetailModal() for
// why: avoids restarting the modal's fadeIn/slideUp entrance animation).
function patchPvpRankTop100Modal(){
  const overlay = document.querySelector('.modal-overlay');
  if(!overlay) return false;
  const bodyEl = overlay.querySelector('.modal-body');
  if(!bodyEl) return false;
  bodyEl.innerHTML = renderPvpRankTop100Body();
  return true;
}
function openPvpRankTop100(){
  showModal(`<img class="ui-icon" src="${ICONS.medal_gold}" alt="🥇"> Top 100 — PvP Rank`, renderPvpRankTop100Body());
  loadPvpRankTop100(false).then(patchPvpRankTop100Modal);
}
