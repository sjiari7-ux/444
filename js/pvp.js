...
const PVP_TIERS = [ // ordered low -> high; used to label a rating
  ...
  { min: 900,  name: 'Silver',   color: '#6b7580' },
  { min: 1100, name: 'Gold',     color: '#b8860b' },
  { min: 1300, name: 'Platinum', color: '#0e7c80' },
  { min: 1500, name: 'Diamond',  color: '#4c5fd6' },
  { min: 1800, name: 'Legend',   color: '#c2410c' },
];
  ...
  return `
    ...
    <div style="max-height:60vh;overflow-y:auto;">
      <table class="lb-table">
        <tr><th>#</th><th>Player</th><th>Rank</th><th>Rating</th></tr>
        ${list.map((r,i)=>{
          const tier = pvpTierFor(r.pvpRating);
          return `<tr class="${r.me?'me':''}">
            <td class="lb-rank">${i+1}</td>
            <td><span style="cursor:pointer;color:var(--brass-bright);" onclick="viewChatProfile('${r.uid}')">${escapeHtml(r.me?'You':r.name)}</span></td>
            <td style="color:${tier.color};font-weight:700;">${tier.name}</td>
            <td style="font-family:'JetBrains Mono',monospace;">${r.pvpRating}</td>
          </tr>`;
        }).join('')}
      </table>
    </div>`;
