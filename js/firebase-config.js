/* ═══════════════════════════════════════════════════════════════
   <img class="ui-icon" src="${ICONS.warning}" alt="⚠">  FIREBASE SETUP REQUIRED
   ═══════════════════════════════════════════════════════════════
...
try {
  if (firebaseConfig.apiKey && !firebaseConfig.apiKey.includes('YOUR_')) {
    if (typeof firebase !== 'undefined' && firebase.initializeApp) {
      ...
      // Used by declareWar()/contributeToWar() in territory.js — those two
      // actions now run server-side (see functions/index.js) instead of
      // writing territories/{tid} directly from the client.
      if (firebase.functions) fns = firebase.functions();
      firebaseReady = true;
    } else {
...
async function enterGame(){
  ...
  if (typeof showDevNoticeModal === 'function') {
    try { showDevNoticeModal(); } catch(e){ console.error('showDevNoticeModal failed:', e); }
  }
}
...
if (auth) {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (errorBox) {
      errorBox.innerHTML = `<img class="ui-icon" src="${ICONS.offline}" alt="🔌"> Could not reach Firebase. Arcadia MMO needs a live connection to play — check your internet connection or disable any ad blocker / privacy extension blocking Google/Firebase, then reload.`;
