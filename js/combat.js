...
function startBattle(zoneId, monsterName){
  ...
  state.classResource = 0; // battle-only meter (Warrior Rage / Archer Precision) starts empty every fight
...
function playerAttackAction(isSkill, skillKey){
  ...
  if(isSkill && skillKey){
    ...
    switch(skillKey){
      ...
      case 'warBanner': manaCost = 12; if(state.mana < manaCost){ pushLog(state, 'Not enough mana!', 'lose'); return; }
        state.mana -= manaCost; dmg = Math.max(1, Math.round(pStats.atk * 2.2)); isPierce = true; break;
...
/* ===== CLASS ULTIMATES (Rage / Precision / Command Points / Fortune) =====
   Gated on isUltimateReady() (state.js) — the button only appears/works
   once the meter is full. Each consumes (some of) its resource. */
function useClassUltimate(){
  if(!battleState || battleState.won || battleState.lost || battleState.fled) return;
  ...
  const ultKey = meta.ultimate.key;
  let dmg = 1;
  switch(ultKey){
    case 'berserkerStrike':
      dmg = Math.max(1, Math.round(pStats.atk * 4 * (1 - (m.def * 0.3) / Math.max(1, pStats.atk))));
      state.classResource = 0;
      break;
    case 'perfectShot':
      dmg = Math.max(1, Math.round(pStats.atk * 2.6)); // ignores defense entirely — full pierce
      state.classResource = 0;
      break;
    case 'rally':
      dmg = Math.max(1, Math.round((pStats.atk - m.def * (1 - (pStats.pierce||0))) * 1.8));
      state.commandPoints = Math.max(0, state.commandPoints - 40);
      break;
    case 'luckyDraw':
      dmg = Math.max(1, Math.round(pStats.atk - m.def * (1 - (pStats.pierce||0))));
      bs.rewards = bs.rewards || {};
      bs.rewards.guaranteedRare = true;
      bs.rewards.bonusGold = Math.round(((m.goldMin||0) + (m.goldMax||0)) / 2 * 0.6);
...
/* Mage-only: Overload Blast — burns double the normal Arcane Power mana
   cost for a much bigger hit + longer burn. Draining Mana to 0 this way
   triggers Exhausted (reduced damage for a few turns) — the Risk/Reward
   loop from the design chat, separate from the shared skillPoints system. */
function castOverloadBlast(){
  if(!battleState || battleState.won || battleState.lost || battleState.fled) return;
  ...
  m.hp -= dmg;
  m.burnTurns = 4;
  m.burnDmg = Math.round(pStats.atk * 0.4);
  updateMissionProgress('hits_landed', 1);
  if(state.mana <= 0){
    state.mageExhaustedTurns = 3;
    pushLog(state, 'Mana drained dry — Exhausted! Damage reduced for a few turns.', 'lose');
  }
...
function afterPlayerAction(){
  ...
  if(state.playerClass === 'mage' && state.mageExhaustedTurns > 0){ state.mageExhaustedTurns--; }
...
function monsterAttackAction(){
  ...
  if(bs.playerHP <= 0){
    bs.playerHP = 0; state.health = 0; bs.lost = true; state.combat.losses += 1;
    updateClassResourceOnBattleEnd(state, false);
    pushLog(state, `${m.name} defeated you!`, 'lose');
  }
...
function awardBattleRewards(){
  ...
  if(bs.rewards && bs.rewards.bonusGold){ gold += bs.rewards.bonusGold; }
  ...
  const guaranteedRare = !!(bs.rewards && bs.rewards.guaranteedRare);
  const dropChance = guaranteedRare ? 1 : (0.30 + (state.playerClass === 'merchant' ? getClassSkillLevel(state, 'lucky') * 0.03 : 0));
  let gearDrop = null;
  if(Math.random() < dropChance){
    const tier = guaranteedRare ? Math.min(GEAR_TIERS.length - 1, rollTier(state.level) + 1) : rollTier(state.level);
    const slots = Object.keys(GEAR_SLOTS); const slot = slots[Math.floor(Math.random()*slots.length)];
    gearDrop = generateGear(slot, tier);
    if(getTotalStorageUsed(state) < getStorageCap(state)){ state.gearBag.push(gearDrop); }
  }
  let gemDrop = 0;
  ...
  gainFortuneFromLootValue(state, gold);
  updateClassResourceOnBattleEnd(state, true);
  bs.rewards = { gold, xp, loot, shards, gearDrop, gemDrop, taxPaid, stolenGold: bs.rewards && bs.rewards.stolenGold ? bs.rewards.stolenGold : 0, guaranteedRare };
...
function renderBattle(){
  ...
  const logLines = bs.turns.slice(-4).map(t => {
    ...
    else if(t.who==='player' && t.type==='flee' && t.success){ txt=`<img class="ui-icon" src="${ICONS.flee}" alt="🏃"> Escaped!`; color='var(--green)'; }
    else if(t.who==='player' && t.type==='flee' && !t.success){ txt=`<img class="ui-icon" src="${ICONS.flee}" alt="🏃"> Flee failed`; color='var(--red)'; }
    ...
    else if(t.who==='monster' && t.dmg>0){ txt=`<img class="ui-icon" src="${ICONS.blood_drop}" alt="🩸"> ${t.dmg}${t.crit?' <img class="ui-icon" src="${ICONS.impact}" alt="💥">':''}`; color='var(--red)'; }
    else if(t.who==='monster' && t.dodge){ txt=`<img class="ui-icon" src="${ICONS.dash}" alt="💨"> Dodged!`; color='var(--skill)'; }
    ...
    else if(t.who==='item' && t.type==='energy'){ txt=`<img class="ui-icon" src="${ICONS.potion}" alt="🧪"> +${t.energy}<img class="ui-icon" src="${ICONS.energy}" alt="⚡">`; color='var(--brass-bright)'; }
  ...
  const resultHtml = bs.won ? `
    <div style="position:absolute;inset:0;background:rgba(5,12,10,0.88);backdrop-filter:blur(4px);z-index:10;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:14px;padding:20px;text-align:center;animation:fadeIn 0.3s ease-out;">
      ...
      <div style="display:flex;flex-direction:column;gap:8px;min-width:220px;">
        ...
        ${bs.rewards.taxPaid ? `<div style="display:flex;justify-content:space-between;font-size:11px;color:var(--dim);padding:0 16px;"><span><img class="ui-icon" src="${ICONS.alliance}" alt="🏛"> Zone tax</span><span>-${bs.rewards.taxPaid}g</span></div>` : ''}
        <div style="display:flex;justify-content:space-between;background:var(--panel-light);border:1px solid var(--border);border-radius:10px;padding:10px 16px;font-size:14px;"><span><img class="ui-icon" src="${ICONS.sparkle}" alt="✨"> XP</span><b style="color:var(--prestige);">${bs.rewards.xp}</b></div>
  ...
  const itemMenuHtml = battleItemMenuOpen ? `
    <div style="position:absolute;bottom:0;left:0;right:0;background:var(--panel);border-top:1px solid var(--border);border-radius:16px 16px 0 0;padding:16px;z-index:20;animation:slideUp 0.25s ease-out;" onclick="event.stopPropagation()">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
        ...
        <button style="background:none;border:none;color:var(--dim);font-size:18px;cursor:pointer;" onclick="toggleBattleItems()"><img class="ui-icon" src="${ICONS.close}" alt="✕"></button>
      </div>
      <div style="display:flex;flex-wrap:wrap;gap:6px;">
        ...
        ${(state.inv.health_potion||0)>0 ? `<button class="mini-btn" style="border-color:var(--health);color:var(--health);padding:8px 12px;" onclick="useBattleItem('health_potion')"><img class="ui-icon" src="${ICONS.heal}" alt="💚"> Health Potion +30-50HP ×${state.inv.health_potion}</button>` : ''}
      </div>
    </div>
  ` : '';
    ...
    <div style="position:absolute;top:0;left:0;right:0;bottom:0;background:rgba(5,12,10,0.92);backdrop-filter:blur(6px);z-index:20;display:flex;align-items:center;justify-content:center;padding:20px;animation:fadeIn 0.2s ease-out;" onclick="if(event.target===this)toggleBattleCodex()">
      <div style="background:var(--panel);border:1px solid var(--border);border-radius:12px;padding:20px;width:100%;max-width:360px;" onclick="event.stopPropagation()">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">
          ...
          <button style="background:none;border:none;color:var(--dim);font-size:18px;cursor:pointer;" onclick="toggleBattleCodex()"><img class="ui-icon" src="${ICONS.close}" alt="✕"></button>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:12px;color:var(--dim);margin-bottom:12px;">
          <div><img class="ui-icon" src="${ICONS.ogre}" alt="👹"> Level: <b style="color:var(--text);">${m.level}</b></div>
          <div><img class="ui-icon" src="${ICONS.heart_hp}" alt="❤️"> HP: <b style="color:var(--text);">${m.maxHp}</b></div>
          <div><img class="ui-icon" src="${ICONS.damage_ui}" alt="⚔"> ATK: <b style="color:var(--text);">${m.atkMin}-${m.atkMax}</b></div>
          <div><img class="ui-icon" src="${ICONS.defense_ui}" alt="🛡"> DEF: <b style="color:var(--text);">${m.def}</b></div>
          <div><img class="ui-icon" src="${ICONS.dash}" alt="💨"> SPD: <b style="color:var(--text);">${m.spd}</b></div>
          <div><img class="ui-icon" src="${ICONS.spellbook}" alt="🎯"> Crit: <b style="color:var(--text);">${(m.crit*100).toFixed(0)}%</b></div>
        </div>
        ...
        <div style="font-size:11px;color:var(--dim);"><img class="ui-icon" src="${ICONS.business}" alt="💰"> Gold: ${m.goldMin}-${m.goldMax}g · <img class="ui-icon" src="${ICONS.sparkle}" alt="✨"> XP: ${m.xp}</div>
      </div>
    </div>
  ` : '';
  ...
  return `<div style="position:relative;display:flex;flex-direction:column;height:100%;min-height:calc(100vh - 60px);background:linear-gradient(180deg,#080b10 0%,#131a24 40%,#1a2530 100%);overflow:hidden;">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;padding:12px 16px;z-index:5;">
      ...
      <div style="display:flex;align-items:center;gap:8px;">
        <div class="${canAct?'animate-pulse':''}" style="font-family:'Cairo',sans-serif;font-weight:800;font-size:11px;letter-spacing:0.06em;padding:5px 10px;border-radius:20px;background:${canAct?'rgba(255,195,92,0.15)':'rgba(196,76,76,0.15)'};color:${canAct?'var(--green)':'var(--red)'};border:1px solid ${canAct?'var(--green)':'var(--red)'};text-transform:uppercase;">${canAct?`<img class="ui-icon" src="${ICONS.energy}" alt="⚡"> Your Turn`:`<img class="ui-icon" src="${ICONS.ogre}" alt="👹"> Enemy Turn`}</div>
    ...
    <div style="flex:1;display:flex;flex-direction:column;justify-content:center;align-items:center;position:relative;padding:0 20px;gap:24px;">
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-end;width:100%;max-width:420px;position:relative;z-index:2;">
        ...
        <div style="display:flex;flex-direction:column;align-items:center;gap:6px;">
          <div id="battle-player-sprite" style="font-size:52px;line-height:1;filter:drop-shadow(0 4px 8px rgba(0,0,0,0.5));">${cls?cls.icon:`<img class="ui-icon" src="${ICONS.class_shield}" alt="🧙">`}</div>
          <!-- HP Bar -->
        ...
        <div style="display:flex;flex-direction:column;align-items:center;gap:4px;">
          ...
          ${m.burnTurns > 0 ? `<div style="font-size:11px;color:var(--copper);font-weight:700;"><img class="ui-icon" src="${ICONS.fire_streak}" alt="🔥"> Burn ${m.burnTurns}t</div>` : ''}
        </div>
    ...
    <div style="padding:12px 16px 24px;display:flex;flex-direction:column;gap:8px;z-index:5;">
      ...
      ${(() => {
        ...
        if(rMeta.key === 'mana'){
          ...
          return `
      <button class="battle-btn" style="margin-top:2px;border-color:${canOverload?rMeta.color:'var(--border)'};color:${canOverload?rMeta.color:'var(--dim)'};${!canOverload?'opacity:0.35;':''}" ${!canOverload?'disabled':''} onclick="castOverloadBlast()">
        ...
        return `
      <button class="battle-btn" style="margin-top:2px;border-color:${ready?rMeta.color:'var(--border)'};color:${ready?rMeta.color:'var(--dim)'};${!ready?'opacity:0.35;':''}${ready?'box-shadow:0 0 10px '+rMeta.color+';':''}" ${!ready?'disabled':''} onclick="useClassUltimate()">
      ...
      <div style="display:flex;justify-content:center;gap:8px;margin-top:4px;">
        <button class="mini-btn" style="font-size:10px;padding:4px 12px;opacity:0.6;" onclick="fleeBattle()"><img class="ui-icon" src="${ICONS.flee}" alt="🏃"> Forfeit</button>
      </div>
    </div>
