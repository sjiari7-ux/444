...
function stateToFirestore(s){
  return {
    ...
    // Persistent class resources (Commander's Command Points feed Kingdom
    // Wars/Core sieges; Merchant's Fortune streak survives across fights).
    commandPoints: s.commandPoints, fortune: s.fortune,
...
function firestoreToState(data){
  ...
  return {
    ...
    classResource: 0, mageExhaustedTurns: 0,
    commandPoints: data.commandPoints || 0, fortune: data.fortune || 0,
    battleLog: [], battleActive: false, battleResult: null,
        // Advanced Market
    marketTab: data.marketTab || 'resources', marketSearch: data.marketSearch || '',
...
/* ═══════════════════════════════════════════════════════════════
   ARCADIA MMO — Citizen Rankings (WarEra-style: pick a metric, pick
   a kingdom to filter to, see tiers + your own rank + a top-20 list)
   ═══════════════════════════════════════════════════════════════ */
const LEADERBOARD_SIZE = 20;
const LEADERBOARD_REFRESH_MS = 30000; // don't hammer Firestore on every tab open

const LB_METRICS = {
  gold:      { field:'gold',       label:'Wealth',     icon:()=>ICONS.gold_coin },
  level:     { field:'level',      label:'Level',      icon:()=>ICONS.medal_silver },
  pvpRating: { field:'pvp.rating', label:'PvP Rating',  icon:()=>ICONS.damage_ui },
  pvpWins:   { field:'pvp.wins',   label:'PvP Wins',    icon:()=>ICONS.damage_ui },
};
// Tier cutoffs, ordered high → low, used purely to label/bucket a value —
// pvpRating reuses the real PVP_TIERS thresholds (pvp.js) so the two
// systems never disagree; the others are cosmetic buckets for this screen.
const LB_TIERS = {
  gold:      [{name:'Master',min:50000,color:'#a78bfa'},{name:'Diamond',min:20000,color:'#7f8ff4'},{name:'Platinum',min:8000,color:'#5fc4c9'},{name:'Gold',min:3000,color:'#d4a843'},{name:'Silver',min:1000,color:'#9aa5ad'},{name:'Bronze',min:0,color:'#a5691f'}],
  level:     [{name:'Master',min:40,color:'#a78bfa'},{name:'Diamond',min:30,color:'#7f8ff4'},{name:'Platinum',min:20,color:'#5fc4c9'},{name:'Gold',min:12,color:'#d4a843'},{name:'Silver',min:6,color:'#9aa5ad'},{name:'Bronze',min:0,color:'#a5691f'}],
  pvpWins:   [{name:'Master',min:100,color:'#a78bfa'},{name:'Diamond',min:50,color:'#7f8ff4'},{name:'Platinum',min:25,color:'#5fc4c9'},{name:'Gold',min:10,color:'#d4a843'},{name:'Silver',min:3,color:'#9aa5ad'},{name:'Bronze',min:0,color:'#a5691f'}],
};
// pvpRating is looked up lazily (not baked into LB_TIERS above) because it
// mirrors PVP_TIERS from pvp.js, which loads AFTER this file — referencing
// it at top-level eval time here would throw before the game even starts.
// By the time anything actually calls lbTiersFor(), every script has run.
function lbTiersFor(metric){
  if(metric === 'pvpRating') return [...PVP_TIERS].reverse().map(t => ({ name:t.name, min:t.min, color:t.color }));
  return LB_TIERS[metric] || LB_TIERS.gold;
}

let citizenLbMetric = 'pvpRating';  // V1: leaderboard is PvP-only (see V1_CHANGES.md); one of LB_METRICS
let citizenLbKingdom = null;        // null = all kingdoms, else a KINGDOMS id
let citizenLbRows = null;           // top-20 rows for the current metric+kingdom
let citizenLbKey = '';              // cache key so switching back doesn't refetch
let citizenLbLoading = false;
let citizenLbError = '';
let citizenLbTierCounts = null;     // {tierName: count} for the current metric+kingdom, or null
let citizenLbMyRank = null;         // your own rank number, or null if not yet known
let citizenLbMetricMenuOpen = false;
let citizenLbKingdomMenuOpen = false;

function metricValueOf(r, metric){
  return metric==='gold' ? r.gold : metric==='level' ? r.level : metric==='pvpRating' ? r.pvpRating : r.pvpWins;
}
function myMetricValue(metric){
  if(metric==='gold') return state.gold||0;
  if(metric==='level') return state.level||1;
  if(metric==='pvpRating') return (state.pvp && state.pvp.rating) || PVP_RATING_DEFAULT;
  return (state.pvp && state.pvp.wins) || 0;
}
function fmtLbValue(metric, val){ return metric==='gold' ? `${fmtG(val)}g` : String(val); }

function toggleCitizenLbMetricMenu(){ citizenLbMetricMenuOpen = !citizenLbMetricMenuOpen; citizenLbKingdomMenuOpen = false; renderBody(); }
function toggleCitizenLbKingdomMenu(){ citizenLbKingdomMenuOpen = !citizenLbKingdomMenuOpen; citizenLbMetricMenuOpen = false; renderBody(); }
function setCitizenLbMetric(m){ citizenLbMetric = m; citizenLbMetricMenuOpen = false; renderBody(); loadCitizenLeaderboard(false).then(renderBody); }
function setCitizenLbKingdom(k){ citizenLbKingdom = k; citizenLbKingdomMenuOpen = false; renderBody(); loadCitizenLeaderboard(false).then(renderBody); }

// Aggregate (`.count()`) queries need Firestore SDK 9.7+ / compat 10.x
// (this project already loads 10.12.0) — wrapped defensively anyway so an
// older SDK or a missing composite index just hides the tier/rank extras
// instead of breaking the top-20 list itself.
async function loadCitizenLeaderboard(force){
  if(!db){ citizenLbError = 'Rankings need a cloud connection.'; return; }
  const key = citizenLbMetric + '|' + (citizenLbKingdom || 'all');
  if(!force && citizenLbRows && citizenLbKey === key) return;
  if(citizenLbLoading) return;
  citizenLbLoading = true;
  citizenLbError = '';
  const metric = LB_METRICS[citizenLbMetric];
  try{
    let q = db.collection('players');
    if(citizenLbKingdom) q = q.where('allianceId', '==', citizenLbKingdom);
    const snap = await q.orderBy(metric.field, 'desc').limit(LEADERBOARD_SIZE).get();
    citizenLbRows = snap.docs.map(playerDocToLbRow);
    citizenLbKey = key;
  }catch(e){
    console.error('[Arcadia Rankings] load failed:', e.code || e.name, e.message);
    citizenLbError = e.code === 'failed-precondition'
      ? "This ranking needs a Firestore index — check the browser console for a one-click link to create it."
      : (e.code === 'permission-denied' ? "Rankings need read access to the players collection." : "Couldn't load rankings right now.");
    citizenLbLoading = false;
    return;
  }

  citizenLbTierCounts = null;
  try{
    const tiers = lbTiersFor(citizenLbMetric);
    const cumulative = await Promise.all(tiers.map(t => {
      let tq = db.collection('players');
      if(citizenLbKingdom) tq = tq.where('allianceId', '==', citizenLbKingdom);
      return tq.where(metric.field, '>=', t.min).count().get();
    }));
    citizenLbTierCounts = {};
    tiers.forEach((t,i) => { citizenLbTierCounts[t.name] = cumulative[i].data().count; });
    for(let i=0;i<tiers.length-1;i++) citizenLbTierCounts[tiers[i].name] -= citizenLbTierCounts[tiers[i+1].name];
  }catch(e){ citizenLbTierCounts = null; } // aggregate queries unsupported/blocked — tier strip just won't show

  citizenLbMyRank = null;
  try{
    let rq = db.collection('players');
    if(citizenLbKingdom) rq = rq.where('allianceId', '==', citizenLbKingdom);
    const c = await rq.where(metric.field, '>', myMetricValue(citizenLbMetric)).count().get();
    citizenLbMyRank = c.data().count + 1;
  }catch(e){ citizenLbMyRank = null; } // same fallback as above — your-rank strip just won't show
...
// Full Top 100 PvP Rank board — separate from the Citizen Rankings above
// (that one tops out at LEADERBOARD_SIZE) so the 4-panel grid on the old
// Leaderboard tab doesn't have to render 100 rows; this one is shown in
// its own modal, opened from the Arena tab's RANK panel. See
// openPvpRankTop100() in pvp.js.
const PVP_RANK_LEADERBOARD_SIZE = 100;
let pvpRankTop100 = null;
let pvpRankTop100Loading = false;
let pvpRankTop100Error = '';
let lastPvpRankTop100Fetch = 0;

async function loadPvpRankTop100(force){
  if(!db){ pvpRankTop100Error = 'The Top 100 needs a cloud connection.'; return; }
  if(pvpRankTop100Loading) return;
  if(!force && pvpRankTop100 && (Date.now() - lastPvpRankTop100Fetch < LEADERBOARD_REFRESH_MS)) return;
  pvpRankTop100Loading = true;
  pvpRankTop100Error = '';
  try{
    const snap = await db.collection('players').orderBy('pvp.rating', 'desc').limit(PVP_RANK_LEADERBOARD_SIZE).get();
    pvpRankTop100 = snap.docs.map(playerDocToLbRow);
    lastPvpRankTop100Fetch = Date.now();
  }catch(e){
    console.error('[Arcadia Leaderboard] Top 100 load failed:', e.code || e.name, e.message);
    pvpRankTop100Error = e.code === 'permission-denied'
      ? "Couldn't load the Top 100 (Firestore rules must allow read access to the players collection)."
      : "Couldn't load the Top 100 right now.";
  }
  pvpRankTop100Loading = false;
}

async function openLeaderboardTab(){
  activeTab = 'leaderboard';
  renderBody();
  await Promise.all([loadCitizenLeaderboard(false), loadKingdomLeaderboard(false)]);
  renderBody();
}

function refreshLeaderboard(){
  Promise.all([loadCitizenLeaderboard(true), loadKingdomLeaderboard(true)])
    .then(()=> (typeof renderBodyUnlessTyping==='function' ? renderBodyUnlessTyping() : renderBody()));
}

/* ═══════════════════════════════════════════════════════════════
   ARCADIA MMO — Kingdoms Leaderboard (the 6 kingdoms ranked by
   territories held, siege record, and total war damage dealt)
   ═══════════════════════════════════════════════════════════════ */
let kingdomLeaderboard = null; // [{id,name,emblem,color,territories,warsWon,warsLost,totalWarDamage,memberCount}] or null
let kingdomLeaderboardLoading = false;
let kingdomLeaderboardError = '';
let lastKingdomLeaderboardFetch = 0;

async function loadKingdomLeaderboard(force){
  if(!db){ kingdomLeaderboardError = 'Kingdoms leaderboard needs a cloud connection.'; return; }
  if(kingdomLeaderboardLoading) return;
  if(!force && kingdomLeaderboard && (Date.now() - lastKingdomLeaderboardFetch < LEADERBOARD_REFRESH_MS)) return;
  kingdomLeaderboardLoading = true;
  kingdomLeaderboardError = '';
  try{
    const [allianceSnap, territorySnap] = await Promise.all([
      db.collection('alliances').get(),
      db.collection('territories').get(),
    ]);
    const allianceById = {};
    allianceSnap.docs.forEach(d => { allianceById[d.id] = d.data(); });
    const territoryCounts = {};
    territorySnap.docs.forEach(d => {
      const owner = d.data().ownerKingdom;
      if(!owner) return;
      territoryCounts[owner] = (territoryCounts[owner] || 0) + 1;
    });
    kingdomLeaderboard = KINGDOMS.map(k => {
      const a = allianceById[k.id] || {};
      return {
        id: k.id, name: k.name, emblem: k.emblem, color: k.color,
        territories: territoryCounts[k.id] || 0,
        warsWon: a.warsWon || 0, warsLost: a.warsLost || 0,
        totalWarDamage: a.totalWarDamage || 0,
        memberCount: a.memberCount || 0,
        mine: k.id === state.allianceId,
      };
    }).sort((x, y) => y.territories - x.territories || y.warsWon - x.warsWon || y.totalWarDamage - x.totalWarDamage);
    lastKingdomLeaderboardFetch = Date.now();
  }catch(e){
    console.error('[Arcadia Leaderboard] Kingdom load failed:', e.code || e.name, e.message);
    kingdomLeaderboardError = e.code === 'permission-denied'
      ? "Couldn't load the kingdoms leaderboard (Firestore rules must allow read access to alliances/territories)."
      : "Couldn't load the kingdoms leaderboard right now.";
  }
  kingdomLeaderboardLoading = false;
}
...
function renderGlobalChatFab(){
  ...
  if(!btn){
    ...
    initChatFabScrollHide(btn);
  }
  const total = chatTotalUnread();
  btn.innerHTML = `<img class="ui-icon" src="${ICONS.chat}" alt="💬">${total>0?`<span class="chat-fab-badge">${total>99?'99+':total}</span>`:''}`;
}

// Fade/shrink the chat FAB while the page is scrolling so it doesn't sit
// on top of row action buttons (Browse/Sell etc.) mid-scroll. It reappears
// a short moment after scrolling stops.
function initChatFabScrollHide(btn){
  let scrollTimer = null;
  const onScroll = ()=>{
    btn.classList.add('is-scrolling');
    clearTimeout(scrollTimer);
    scrollTimer = setTimeout(()=> btn.classList.remove('is-scrolling'), 350);
  };
  ...
  modal.innerHTML = `
    <div class="modal-overlay" style="z-index:120;" onclick="if(event.target===this)closeChatProfile()">
      <div class="modal-box" style="max-width:480px;">
        <div class="modal-header"><h3><img class="ui-icon" src="${ICONS.player_icon}" alt="👤"> Player Profile</h3><button class="modal-close" onclick="closeChatProfile()"><img class="ui-icon" src="${ICONS.close}" alt="✕"></button></div>
        <div class="modal-body" style="padding:30px 20px;text-align:center;color:var(--dim);font-size:13px;">Loading…</div>
      </div>
    </div>`;
  try{
    const doc = await db.collection('players').doc(uid).get();
    ...
    if(!doc.exists){
      box.innerHTML = `<div class="modal-header"><h3><img class="ui-icon" src="${ICONS.player_icon}" alt="👤"> Player Profile</h3><button class="modal-close" onclick="closeChatProfile()"><img class="ui-icon" src="${ICONS.close}" alt="✕"></button></div>
        <div class="modal-body" style="padding:30px 20px;text-align:center;color:var(--dim);font-size:13px;">This player no longer exists.</div>`;
      return;
    }
    const d = doc.data();
    const kingdom = d.allianceId ? kingdomDef(d.allianceId) : null;
    const roleDef = d.allianceRole ? ALLIANCE_ROLES[d.allianceRole] : null;
    box.innerHTML = `<div class="modal-header"><h3><img class="ui-icon" src="${ICONS.player_icon}" alt="👤"> ${escapeHtml(d.username||'Player')}</h3><button class="modal-close" onclick="closeChatProfile()"><img class="ui-icon" src="${ICONS.close}" alt="✕"></button></div>
      <div class="modal-body">${buildPlayerProfileHtml({
        username: d.username || 'Player',
        avatar: d.avatar || '🧙',
        level: d.level || 1,
        playerClass: d.playerClass || d.class || null,
        wins: (d.pvp && d.pvp.wins) || 0,
        losses: (d.pvp && d.pvp.losses) || 0,
        gold: d.gold || 0,
        allianceName: kingdom ? kingdom.name : null,
        allianceRole: roleDef ? roleDef.name : null,
        bio: d.bio || '',
        gearCount: (d.gearBag && d.gearBag.length) || 0,
        totalGoldEarned: d.totalGoldEarned || d.gold || 0,
        prestigePoints: (d.prestige && d.prestige.points) || 0,
      }, false)}</div>`;
  }catch(e){
    console.error('[Arcadia Chat] Failed to load profile:', e);
    const box = modal.querySelector('.modal-box');
    if(box) box.innerHTML = `<div class="modal-header"><h3><img class="ui-icon" src="${ICONS.player_icon}" alt="👤"> Player Profile</h3><button class="modal-close" onclick="closeChatProfile()"><img class="ui-icon" src="${ICONS.close}" alt="✕"></button></div>
      <div class="modal-body" style="padding:30px 20px;text-align:center;color:var(--dim);font-size:13px;">Couldn't load this profile.</div>`;
  }
}
  ...
  return `
    <div class="chat-drawer-tabs">
      ...
      <button class="chat-drawer-tab ${chatActiveTab==='alliance'?'active':''} ${hasAlliance?'':'disabled'}" title="${hasAlliance?'':'Join a kingdom to unlock'}" onclick="${hasAlliance?"switchChatTab('alliance')":''}">
        <img class="ui-icon" src="${ICONS.castle}" alt="🏰"> Kingdom${allianceUnreadCount>0 && chatActiveTab!=='alliance' ? `<span class="tab-dot">${allianceUnreadCount>9?'9+':allianceUnreadCount}</span>` : ''}
      </button>
    </div>`;
}
...
function renderChatDrawerHTML(){
  if(!db || !UID){
    return `
      <div class="chat-drawer-header">
        <h3><img class="ui-icon" src="${ICONS.chat}" alt="💬"> Chat</h3>
        <button class="modal-close" onclick="closeChatDrawer()"><img class="ui-icon" src="${ICONS.close}" alt="✕"></button>
      </div>
  ...
  const title = chatActiveTab === 'alliance' ? `<img class="ui-icon" src="${ICONS.castle}" alt="🏰"> Kingdom Chat` : `<img class="ui-icon" src="${ICONS.globe}" alt="🌐"> Global Chat`;
