...
// Full Top 100 PvP Rank board — separate from the top-20 preview above
// (leaderboardByPvpRating) so the 4-panel grid on the Leaderboard tab
// doesn't have to render 100 rows; this one is shown in its own modal,
// opened from the Arena tab's RANK panel. See openPvpRankTop100() in pvp.js.
...
async function openLeaderboardTab(){
  ...
  renderBody();
  await Promise.all([loadRealLeaderboard(false), loadKingdomLeaderboard(false)]);
  renderBody();
}

function refreshLeaderboard(){
  Promise.all([loadRealLeaderboard(true), loadKingdomLeaderboard(true)])
