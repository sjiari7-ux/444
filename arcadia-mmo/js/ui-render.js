...
function renderZoneView(){
  ...
  if(!z) return '';
  const isCraft = state.zoneSubTab==='craft';
  ...
  return `<div class="wrap animate-fade">
    ...
    <div style="display:flex;gap:0;margin-bottom:16px;background:var(--bg);border:1px solid var(--border);border-radius:8px;overflow:hidden;">
      <button onclick="setZoneSubTab('craft')" style="flex:1;padding:10px 8px;border:none;background:${isCraft?'var(--panel-light)':'transparent'};color:${isCraft?'var(--green)':'var(--dim)'};font-family:'Cairo',sans-serif;font-weight:700;font-size:13px;cursor:pointer;border-right:1px solid var(--border);position:relative;">
        ⚒️ Craft
        ${isCraft?'<div style="position:absolute;bottom:0;left:15%;width:70%;height:2px;background:var(--green);border-radius:1px;"></div>':''}
      </button>
      <button onclick="setZoneSubTab('dungeon')" style="flex:1;padding:10px 8px;border:none;background:${!isCraft?'var(--panel-light)':'transparent'};color:${!isCraft?'var(--red)':'var(--dim)'};font-family:'Cairo',sans-serif;font-weight:700;font-size:13px;cursor:pointer;position:relative;">
        ⚔️ Dungeon
        ${!isCraft?'<div style="position:absolute;bottom:0;left:15%;width:70%;height:2px;background:var(--red);border-radius:1px;"></div>':''}
      </button>
    </div>
